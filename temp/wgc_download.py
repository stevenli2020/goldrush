...
# All 5 target configurations
TARGETS = [
    {
        "name": "Above Ground Gold",
        "url": "https://www.gold.org/goldhub/data/how-much-gold",
        "pattern": r'href=["\'](.*?/download/file/.*?above-ground-gold.*?\.xlsx?)\s*["\']',
    },
    {
        "name": "Gold ETF Holdings and Flows",
        "url": "https://www.gold.org/goldhub/data/gold-etfs-holdings-and-flows",
        "pattern": r'href=["\'](.*?/download/file/.*?(?:etf.*?flow|flow.*?etf).*?\.xlsx?)\s*["\']',
    },
    {
        "name": "Gold Demand by Country (GDT)",
        "url": "https://www.gold.org/goldhub/data/gold-demand-by-country",
        "pattern": r'href=["\'](.*?/download/file/.*?gdt.*?\.xlsx?)\s*["\']',
    },
    {
        "name": "World Official Gold Reserves",
        "url": "https://www.gold.org/goldhub/data/gold-reserves-by-country",
        "pattern": r'href=["\'](.*?/download/file/.*?world.*?official.*?\.xlsx?)\s*["\']',
    },
    {
        "name": "Gold Reserves Changes",
        "url": "https://www.gold.org/goldhub/data/gold-reserves-by-country",
        "pattern": r'href=["\'](.*?/download/file/.*?change.*?\.xlsx?)\s*["\']',
    },
]
...
def process_download(session, target, page_cache):
    ...
    # Reuse cached HTML if available for this URL to avoid extra network requests
    if page_url in page_cache:
        ...
        page_response = session.get(
            page_url, headers=headers, impersonate="chrome"
        )
    ...
    # Search HTML for target download link
    ...
    # Strip any stray whitespace captured inside the href (some gold.org
    # download links have trailing spaces before the closing quote).
...
def scrape_and_download():
    ...
    for c in cookie_list:
        session.cookies.set(
            c["name"], c["value"], domain=c.get("domain", ".gold.org")
        )

    # Dictionary to cache HTML pages by URL across operations
    page_cache = {}
    ...
    for target in TARGETS:
        # Run the download and capture if it succeeded
        ...
        # Save session cookies after every operation
