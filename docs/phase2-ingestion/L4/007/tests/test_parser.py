...
def write_inputs(tmp_path, observations, *, series_id='GFDEGDQ188S', bad_metadata=False):
...
def test_valid_quarterly_parsing_and_semantics(tmp_path):
    raw, manifest = write_inputs(tmp_path, [{'date': '2025-10-01', 'value': '122.56815'}, {'date': '2026-01-01', 'value': '122.59387'}])
...
def test_missing_marker_and_chronological_order(tmp_path):
    raw, manifest = write_inputs(tmp_path, [{'date': '2026-01-01', 'value': '122.5'}, {'date': '2025-07-01', 'value': '.'}, {'date': '2025-10-01', 'value': '121.4'}])
    rows = parser.parse_observations(raw, manifest)
    assert [row['observation_date'] for row in rows] == ['2025-10-01', '2026-01-01']

@pytest.mark.parametrize('observation_date,value', [('bad', '122'), ('2026-02-01', '122'), ('2026-01-02', '122'), ('2026-01-01', 'bad'), ('2026-01-01', 'nan'), ('2026-01-01', 'inf')])
...
def test_duplicate_date_behavior(tmp_path):
    raw, manifest = write_inputs(tmp_path, [{'date': '2026-01-01', 'value': '122'}, {'date': '2026-01-01', 'value': '122'}])
    assert len(parser.parse_observations(raw, manifest)) == 1
    raw, manifest = write_inputs(tmp_path, [{'date': '2026-01-01', 'value': '122'}, {'date': '2026-01-01', 'value': '123'}])
...
def test_prior_fallback_is_one_schema_valid_stale_row(tmp_path):
    raw, manifest = write_inputs(tmp_path, [{'date': '2025-10-01', 'value': '122.5'}, {'date': '2026-01-01', 'value': '122.6'}])
...
def test_blocked_then_successful_recovery(tmp_path):
    ...
    blocked = subprocess.run([sys.executable, str(PACKAGE / 'parser.py'), '--raw', str(bad_raw), '--manifest', str(bad_manifest), '--output', str(output)], capture_output=True, text=True, check=False)
