...
OUTPUT_FIELDS = [
    "variable_id", "observation_date", "observation_year", "observation_quarter",
    "federal_debt_pct_gdp", "unit", "series_definition", "period_definition",
    "seasonal_adjustment", "source_name", "source_series_id", "source_release",
    "raw_file_path", "manifest_path", "raw_sha256", "retrieved_at",
...
def load_inputs(raw_path: Path, manifest_path: Path) -> tuple[list[dict[str, Any]], dict[str, Any], str]:
    ...
    return observations, manifest, digest


def quarter_end(observation_date: date) -> date:
    ends = {1: date(observation_date.year, 3, 31), 4: date(observation_date.year, 6, 30),
            7: date(observation_date.year, 9, 30), 10: date(observation_date.year, 12, 31)}
    try:
...
def parse_observations(
    ...
    *,
    ...
    observations, manifest, digest = load_inputs(raw_path, manifest_path)
    ...
    availability = operational_availability(
        max(by_date), today=current_date, stale_after_days=stale_after_days
    )
    ...
    for observation_date in sorted(by_date):
        ...
        rows.append({
            ...
            "observation_year": observation_date.year,
            "observation_quarter": ((observation_date.month - 1) // 3) + 1,
            "federal_debt_pct_gdp": value,
            "unit": "percent_of_gdp",
            "series_definition": "U.S. federal total public debt divided by GDP",
            "period_definition": "FRED quarterly observation date is the first day of the quarter",
            "seasonal_adjustment": "seasonally_adjusted",
            "source_name": "FRED",
            "source_series_id": SERIES_ID,
            "source_release": "Debt to Gross Domestic Product Ratios",
            "raw_file_path": str(raw_path),
            "manifest_path": str(manifest_path),
            "raw_sha256": digest,
            "retrieved_at": retrieved_at,
            "validation_status": "PASS" if 0 <= value <= 250 else "FLAG",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.raw and args.manifest:
            rows = parse_observations(args.raw, args.manifest,
