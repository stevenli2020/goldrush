...
VARIABLE_ID, SERIES_ID, UNIT, VERSION = "L4-001", "CPIAUCSL", "index", "0.1.0"
...
if __name__ == "__main__":
    ap=argparse.ArgumentParser(); ap.add_argument("--raw", type=Path, required=True); ap.add_argument("--output", type=Path, default=Path("data/processed/L4_001_observations.csv")); ap.add_argument("--retrieved-at"); args=ap.parse_args(); rows=parse(args.raw, args.retrieved_at); write_csv(rows,args.output); print(f"Wrote {len(rows)} observations to {args.output}")
