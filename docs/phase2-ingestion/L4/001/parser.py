...
def parse(raw_path: Path, retrieved_at=None, stale_after_days=45):
    ...
    for item in items:
        ...
        rows.append({'variable_id': VARIABLE_ID, 'observation_date': date.isoformat(), 'value': value, 'unit': UNIT, 'source_name': 'FRED', 'source_series_id': SERIES_ID, 'raw_file_path': str(raw_path), 'retrieved_at': retrieved_at, 'validation_status': 'PASS' if value > 0 else 'FLAG', 'availability_status': 'STALE' if (now.date() - date).days > stale_after_days else 'AVAILABLE', 'parser_version': VERSION})
    if not rows:
...
if __name__ == '__main__':
    ...
    ap.add_argument('--output', type=Path, default=Path('data/processed/L4_001_observations.csv'))
