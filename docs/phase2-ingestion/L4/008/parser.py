...
REQUIRED = {'130': 'Total Receipts', '360': 'Interest on Treasury Debt Securities (Gross)'}
OUTPUT_FIELDS = ['variable_id', 'observation_date', 'fiscal_year', 'gross_interest_expense_usd', 'total_receipts_usd', 'interest_expense_to_revenue_pct', 'unit', 'accounting_convention', 'source_name', 'endpoint', 'query', 'raw_file_paths', 'manifest_path', 'retrieved_at', 'validation_status', 'availability_status', 'parser_version']
...
def parse_manifest(manifest_path: Path, *, stale_after_days: int=450) -> list[dict[str, Any]]:
    ...
    for (date_text, fiscal_year), values in sorted(pairs.items()):
        ...
        rows.append({'variable_id': VARIABLE_ID, 'observation_date': date_text, 'fiscal_year': fiscal_year, 'gross_interest_expense_usd': float(interest), 'total_receipts_usd': float(receipts), 'interest_expense_to_revenue_pct': ratio_float, 'unit': 'percent_of_federal_receipts', 'accounting_convention': 'Gross interest on Treasury debt securities / total federal receipts, fiscal-year-to-date at September month-end', 'source_name': 'U.S. Treasury Fiscal Data — Monthly Treasury Statement Table 3', 'endpoint': ENDPOINT, 'query': query_text, 'raw_file_paths': raw_text, 'manifest_path': str(manifest_path), 'retrieved_at': manifest['retrieved_at'], 'validation_status': validation, 'availability_status': availability, 'parser_version': PARSER_VERSION})
    return rows
