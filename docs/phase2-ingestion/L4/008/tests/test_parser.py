...
def row(date, year, code, amount, description=None):
    ...
    return {
        "record_date": date, "record_fiscal_year": str(year), "line_code_nbr": code,
        "classification_desc": description or descriptions[code],
        "current_fytd_rcpt_outly_amt": str(amount),
    }
...
class L4008ParserTests(unittest.TestCase):
    def fixture(self, directory, rows, **manifest_changes):
        ...
        manifest = {
            "endpoint": ENDPOINT,
            "query": {"filter": "line_code_nbr:in:(130,360)", "page[size]": 1000},
            "retrieved_at": "2026-08-24T00:00:00+00:00", "raw_paths": [str(raw)],
            "page_sha256": [digest], "source_sha256": digest,
        }
    ...
    def valid_rows(self):
        return [
            row("2024-08-31", 2024, "130", 300), row("2024-08-31", 2024, "360", 60),
            row("2024-09-30", 2024, "130", 500), row("2024-09-30", 2024, "360", 100),
            row("2025-09-30", 2025, "130", 600), row("2025-09-30", 2025, "360", 150),
        ]
    ...
    def test_invalid_values_dates_and_hash_fail(self):
        cases = [
            [row("bad", 2025, "130", 600), row("2025-09-30", 2025, "360", 150)],
            [row("2025-09-30", 2025, "130", 0), row("2025-09-30", 2025, "360", 150)],
            [row("2025-09-30", 2025, "130", -1), row("2025-09-30", 2025, "360", 150)],
            [row("2025-09-30", 2025, "130", "NaN"), row("2025-09-30", 2025, "360", 150)],
        ]
        for rows in cases:
    ...
    def test_fallback_output_is_one_schema_valid_stale_row(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for field in ("fiscal_year",): typed[field] = int(typed[field])
            for field in ("gross_interest_expense_usd", "total_receipts_usd", "interest_expense_to_revenue_pct"): typed[field] = float(typed[field])
