...
VARIABLE_ID, SERIES_ID, UNIT, VERSION = "L4-004", "T10YIE", "percent", "0.1.0"
...
def parse(raw_path: Path, retrieved_at=None, stale_after_days=7):
    ...
    for item in items:
        ...
        rows.append({"variable_id":VARIABLE_ID,"observation_date":date.isoformat(),"value":value,"unit":UNIT,"source_name":"FRED","source_series_id":SERIES_ID,"raw_file_path":str(raw_path),"raw_sha256":digest,"retrieved_at":retrieved_at,"validation_status":"PASS" if -10<=value<=20 else "FLAG","availability_status":"STALE" if (now.date()-date).days>stale_after_days else "AVAILABLE","parser_version":VERSION})
...
if __name__=="__main__":
    ap=argparse.ArgumentParser(); ap.add_argument("--raw",type=Path,required=True); ap.add_argument("--output",type=Path,default=Path("data/processed/L4_004_observations.csv")); ap.add_argument("--retrieved-at"); args=ap.parse_args(); rows=parse(args.raw,args.retrieved_at); write_csv(rows,args.output); print(f"Wrote {len(rows)} observations to {args.output}")
