...
def source_row(record_date, line, maturity, amount, class_desc='Notes', type_desc='Marketable'):
    return {'record_date': record_date, 'security_type_desc': type_desc, 'security_class1_desc': class_desc, 'maturity_date': maturity, 'outstanding_amt': amount, 'src_line_nbr': str(line)}

def typed(row):
    result = dict(row)
    for field in ('maturing_within_1y_mil_usd', 'total_marketable_outstanding_mil_usd', 'dated_detail_outstanding_mil_usd', 'classification_coverage_pct', 'marketable_debt_maturing_within_1y_pct'):
...
class L4009ParserTests(unittest.TestCase):
    ...
    def fixture(self, directory, pages, **manifest_changes):
        ...
        manifest = {'endpoint': ENDPOINT, 'query': {'fields': FIELDS, 'filter': 'record_date:gte:1900-01-01', 'page[size]': 1000}, 'retrieved_at': '2026-08-24T00:00:00+00:00', 'raw_paths': raw_paths, 'page_count': len(pages)}
    ...
    def valid_rows(self, record_date='2024-02-29'):
        return [source_row(record_date, 1, record_date, '50'), source_row(record_date, 2, '2024-03-01', '100'), source_row(record_date, 3, '2025-02-28', '200'), source_row(record_date, 4, '2025-03-01', '620'), source_row(record_date, 5, 'null', '970', 'Notes Total'), source_row(record_date, 6, 'null', '30', 'Federal Financing Bank'), source_row(record_date, 7, 'null', '1000', 'Total Marketable'), source_row(record_date, 8, '2024-03-01', 'null'), source_row(record_date, 9, ' * ', '*  ')]
    ...
    def test_bill_maturity_value_can_make_coverage_exceed_100(self):
        with tempfile.TemporaryDirectory() as directory:
            rows = [source_row('2024-01-31', 1, '2024-06-30', '30', 'Bills Maturity Value'), source_row('2024-01-31', 2, '2026-01-31', '75', 'Notes'), source_row('2024-01-31', 3, 'null', '100', 'Total Marketable')]
    ...
    def test_invalid_dates_amounts_and_bounds_fail(self):
        cases = []
        bad_date = self.valid_rows()
        bad_date[1] = source_row('2024-02-29', 2, 'bad', '100')
        cases.append(bad_date)
        bad_record = self.valid_rows()
        bad_record[1] = source_row('bad', 2, '2024-03-01', '100')
        cases.append(bad_record)
        negative = self.valid_rows()
        negative[1] = source_row('2024-02-29', 2, '2024-03-01', '-1')
        cases.append(negative)
        nan = self.valid_rows()
        nan[1] = source_row('2024-02-29', 2, '2024-03-01', 'NaN')
        cases.append(nan)
        too_large = self.valid_rows()
        too_large[1] = source_row('2024-02-29', 2, '2024-03-01', '1001')
