...
def _raw_rows(manifest: dict[str, Any], manifest_path: Path) -> tuple[list[dict[str, Any]], list[Path]]:
    ...
    paths = []
    for raw_text in manifest['raw_paths']:
...
def parse_manifest(manifest_path: Path, *, stale_after_days: int=62) -> list[dict[str, Any]]:
    ...
    for observation_date, items in sorted(grouped.items()):
        ...
        output.append({'variable_id': VARIABLE_ID, 'observation_date': observation_date.isoformat(), 'maturing_within_1y_mil_usd': float(within_year), 'total_marketable_outstanding_mil_usd': float(denominator), 'dated_detail_outstanding_mil_usd': float(dated_total), 'classification_coverage_pct': coverage_float, 'marketable_debt_maturing_within_1y_pct': ratio_float, 'unit': 'percent_of_marketable_treasury_debt', 'measure_definition': 'Positive dated marketable debt maturing after record date and within one calendar year / Total Marketable outstanding * 100', 'source_name': 'U.S. Treasury Fiscal Data — Monthly Statement of the Public Debt Table 3', 'endpoint': ENDPOINT, 'query': query_text, 'raw_file_paths': raw_text, 'manifest_path': str(manifest_path), 'page_count': len(raw_paths), 'retrieved_at': manifest['retrieved_at'], 'validation_status': validation, 'availability_status': availability, 'parser_version': PARSER_VERSION})
    return output
