...
def write_inputs(tmp_path, observations, *, series_id="FYFSGDA188S", bad_hash=False):
...
def test_wrong_series_id_and_hash_mismatch(tmp_path):
    raw, wrong = write_inputs(tmp_path, [{"date": "2025-01-01", "value": "-5"}], series_id="GDPA")
    with pytest.raises(ValueError, match="series_id"):
        parser.parse_observations(raw, wrong)
    raw, bad_hash = write_inputs(tmp_path, [{"date": "2025-01-01", "value": "-5"}], bad_hash=True)
...
def test_blocked_then_successful_recovery(tmp_path):
    ...
    command = [sys.executable, str(PACKAGE / "parser.py"), "--raw", str(bad_raw),
...
def test_cli_failure_with_prior_writes_one_stale_row(tmp_path):
    good_raw, good_manifest = write_inputs(tmp_path, [
        {"date": "2024-01-01", "value": "-6.2"},
        {"date": "2025-01-01", "value": "-5.7"},
    ])
    prior = tmp_path / "prior.csv"
