...
if __name__=="__main__":
    ap=argparse.ArgumentParser(); ap.add_argument("--raw",type=Path,required=True); ap.add_argument("--output",type=Path,default=Path("data/processed/L4_003_observations.csv")); ap.add_argument("--retrieved-at"); args=ap.parse_args(); rows=parse(args.raw,args.retrieved_at); write_csv(rows,args.output); print(f"Wrote {len(rows)} observations to {args.output}")
