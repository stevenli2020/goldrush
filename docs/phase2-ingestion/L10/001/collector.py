...
def extract_gold_rows(text: str, *, raw_sha256: str, raw_path: str, retrieved_at: str) -> list[dict]:
    rows: list[dict] = []
    for line_number, fields in enumerate(csv.reader(io.StringIO(text)), start=1):
        ...
        rows.append({
            ...
            "market_name": fields[0],
            "cftc_contract_market_code": fields[3],
            "open_interest": _int_field(fields[7], "open_interest"),
            "managed_money_long": _int_field(fields[13], "managed_money_long"),
            "managed_money_short": _int_field(fields[14], "managed_money_short"),
            "managed_money_spreading": _int_field(fields[15], "managed_money_spreading"),
            "contract_units": fields[185],
            "fut_only_or_combined": fields[190],
            "raw_path": raw_path,
...
def download_bytes(url: str = SOURCE_URL) -> tuple[bytes, int]:
    completed = subprocess.run(
        ["curl", "--fail", "--location", "--silent", "--show-error", "--user-agent", "GoldRush/0.1", "--write-out", "\\n%{http_code}", url],
        check=True,
        capture_output=True,
    )
    try:
...
def collect(raw_dir: Path, manifest_dir: Path, extracted_path: Path, *, url: str = SOURCE_URL) -> dict:
    ...
    rows = extract_gold_rows(data.decode("utf-8-sig"), raw_sha256=digest, raw_path="", retrieved_at=retrieved_at)
    report_date = max(row["report_date"] for row in rows)
    raw_path = raw_dir / f"f_disagg_{report_date}_{digest[:12]}.txt"
    for row in rows:
    ...
    manifest = {
        ...
        "size_bytes": len(data),
        "sha256": digest,
        "report_date_latest": report_date,
        "gold_row_count": len(rows),
        "http_status": http_status,
        "client_version": CLIENT_VERSION,
    }
    manifest_path = manifest_dir / f"f_disagg_{report_date}_{digest[:12]}_{retrieval_stamp}.manifest.json"
