...
def test_extracts_locked_gold_contract():
    rows = collector.extract_gold_rows(",".join(make_row()), raw_sha256="a" * 64, raw_path="raw.txt", retrieved_at="2026-08-21T00:00:00+00:00")
...
def test_ignores_other_contracts():
    ...
    assert len(collector.extract_gold_rows(text, raw_sha256="a" * 64, raw_path="raw.txt", retrieved_at="2026-08-21T00:00:00+00:00")) == 1


def test_rejects_wrong_layout():
    with pytest.raises(ValueError, match="191 fields"):
        collector.extract_gold_rows(",".join(["x"] * 190), raw_sha256="a" * 64, raw_path="raw.txt", retrieved_at="2026-08-21T00:00:00+00:00")


def test_rejects_missing_required_gold_field():
    with pytest.raises(ValueError, match="missing required"):
        collector.extract_gold_rows(",".join(make_row(missing=True)), raw_sha256="a" * 64, raw_path="raw.txt", retrieved_at="2026-08-21T00:00:00+00:00")
...
def test_rejects_conflicting_duplicate_report_date():
    ...
    with pytest.raises(ValueError, match="conflicting duplicate"):
        collector.extract_gold_rows(",".join(first) + "\n" + ",".join(second), raw_sha256="a" * 64, raw_path="raw.txt", retrieved_at="2026-08-21T00:00:00+00:00")
