...
def parse(source: Path, *, as_of: str | None=None, prior: Path | None=None) -> list[dict]:
    ...
    with source.open(newline='', encoding='utf-8') as handle:
        for row in csv.DictReader(handle):
            ...
            is_revision = bool(prior_row and any((prior_row.get(processed_name, prior_row.get(source_name, '')) != row[source_name] for source_name, processed_name in (('managed_money_long', 'managed_money_long_contracts'), ('managed_money_short', 'managed_money_short_contracts'), ('managed_money_spreading', 'managed_money_spreading_contracts'), ('open_interest', 'open_interest_contracts')))))
            age = (today - report_date).days
            availability = 'AVAILABLE' if age <= STALE_DAYS else 'STALE'
            output.append({'variable_id': 'L10-001', 'observation_date': row['report_date'], 'report_date': row['report_date'], 'value': net, 'managed_money_net_contracts': net, 'managed_money_long_contracts': long, 'managed_money_short_contracts': short, 'managed_money_spreading_contracts': spreading, 'open_interest_contracts': open_interest, 'unit': 'contracts', 'source_name': 'CFTC Disaggregated Futures-Only COT', 'source_series_id': 'CFTC_088691_FutOnly', 'market_name': row['market_name'], 'cftc_contract_market_code': row['cftc_contract_market_code'], 'fut_only_or_combined': row['fut_only_or_combined'], 'raw_path': row['raw_path'], 'retrieved_at': row['retrieved_at'], 'validation_status': validation, 'availability_status': availability, 'is_revision': is_revision, 'parser_version': PARSER_VERSION})
