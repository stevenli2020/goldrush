...
class L9001Pipeline:
  ...
  def extract(self) -> str:
    ...
    response = requests.get(
        self.TARGET_URL, headers=self.headers, timeout=self.timeout
    )
  ...
  def transform(self, raw_html: str) -> pd.DataFrame:
    ...
    df["Benchmark Price AM"] = pd.to_numeric(
        df["Benchmark Price AM"], errors="coerce"
    )
    df["Benchmark Price PM"] = pd.to_numeric(
        df["Benchmark Price PM"], errors="coerce"
    )
...
def test_l9_001_pipeline():
  ...
  for col in L9001Pipeline.EXPECTED_COLUMNS:
    assert col in df.columns, (
        f"Assertion Error: Missing expected column '{col}'"
    )
