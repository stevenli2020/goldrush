...
# Default directory path as specified
...
def resolve_filepath(default_path: str = DEFAULT_FILE_PATH) -> str:
    ...
    return default_path  # Default return; pandas will raise FileNotFoundError if missing
...
def execute_l9_004_ingestion(filepath: str) -> pd.DataFrame:
    ...
    if not os.path.exists(filepath):
        raise FileNotFoundError(
            f"Unable to locate target workbook at '{filepath}'. "
            f"Please verify that the file exists in 'data/gold-demand-trends/'."
        )

    xls = pd.ExcelFile(filepath)

    # --- 1. Parse 'India Supply' Sheet ---
    df_supply_raw = pd.read_excel(xls, sheet_name="India Supply")
    periods_supply = df_supply_raw.iloc[3].values
    q_cols_supply = [
        idx
        for idx, val in enumerate(periods_supply)
        if isinstance(val, str)
        and (
            val.startswith("Q1")
            or val.startswith("Q2")
            or val.startswith("Q3")
            or val.startswith("Q4")
        )
        and "Year" not in val
    ]
    ...
    india_jewel_row = df_jewel_raw[
        df_jewel_raw.iloc[:, 1].astype(str).str.strip().str.lower() == "india"
    ]
    jewel_vals = pd.to_numeric(
        india_jewel_row.iloc[0, q_cols_jewel].values, errors="coerce"
    )
    s_jewel = pd.Series(
        jewel_vals, index=[periods_jewel[i] for i in q_cols_jewel]
    )

    # --- 3. Parse 'Bar and Coin' Demand (India) ---
    df_bar_raw = pd.read_excel(xls, sheet_name="Bar and Coin")
    periods_bar = df_bar_raw.iloc[3].values
    q_cols_bar = [
        idx
        for idx, val in enumerate(periods_bar)
        if isinstance(val, str)
        and (
            val.startswith("Q1")
            or val.startswith("Q2")
    ...
    india_bar_row = df_bar_raw[
        df_bar_raw.iloc[:, 1].astype(str).str.strip().str.lower() == "india"
    ]
    bar_vals = pd.to_numeric(
        india_bar_row.iloc[0, q_cols_bar].values, errors="coerce"
    )
    s_bar = pd.Series(bar_vals, index=[periods_bar[i] for i in q_cols_bar])

    # --- 4. Parse 'Gold Prices' (INR / 10g) ---
    df_prices_raw = pd.read_excel(xls, sheet_name="Gold Prices")
    q_cols_prices = [
        c
        for c in range(df_prices_raw.shape[1])
        if str(df_prices_raw.iloc[3, c]).startswith("Q")
    ]
    q_labels_prices = [df_prices_raw.iloc[3, c] for c in q_cols_prices]
    price_vals = pd.to_numeric(
        [df_prices_raw.iloc[9, c] for c in q_cols_prices], errors="coerce"
    )
    s_prices = pd.Series(price_vals, index=q_labels_prices)

    # --- 5. Assemble Master Dataset & Derived Indicators ---
    df_master = df_supply.copy()
    df_master["jewellery_demand_t"] = s_jewel
    df_master["bar_coin_demand_t"] = s_bar
    df_master["total_consumer_demand_t"] = (
        df_master["jewellery_demand_t"] + df_master["bar_coin_demand_t"]
    )
    df_master["gold_price_inr_10g"] = s_prices

    # Key derived ratios
    df_master["import_coverage_ratio_pct"] = (
        df_master["gross_imports_t"] / df_master["total_consumer_demand_t"]
    ) * 100
    df_master["dore_import_share_pct"] = (
        df_master["dore_imports_t"] / df_master["gross_imports_t"]
    ) * 100
...
def run_l9_004_ingestion_test() -> None:
    ...
    try:
        ...
        # --- 2. DISPLAY ACTUAL DATA (TABULAR FORM) ---
        print("\n" + "-" * 80)
        ...
        display_cols = [
            ...
            "jewellery_demand_t",
            "bar_coin_demand_t",
            "total_consumer_demand_t",
            "gold_price_inr_10g",
            "import_coverage_ratio_pct",
        ]

        df_subset = df.tail(10)[display_cols].round(2)
        print(df_subset.to_string())

        # --- 3. DISPLAY ACTUAL DATA (JSON PAYLOAD STRUCTURE) ---
        print("\n" + "-" * 80)
        ...
        payload = {
            ...
            "period": latest_q,
            "geography": "India",
            "data_type": "Physical Gold Ingestion",
