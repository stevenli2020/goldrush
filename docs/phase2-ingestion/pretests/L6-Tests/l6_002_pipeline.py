...
def parse_and_engineer_l6_002(file_path):
  ...
  # 1. Clean Target Type (Map '0', '-', and blank to 'Entity')
  ...
  # 2. String preparation for Geopolitical Exposure
  df["program_str"] = df["program"].fillna("").astype(str)
  df["remarks_str"] = df["remarks"].fillna("").astype(str)

  # 3. Targeted Geopolitical Program & Country Exposure
  russia_ukr_mask = df["program_str"].str.contains(
      "UKRAINE|RUSSIA|RUSHAR|EO14024|EO13661|PEESA|CAATSA", case=False
  )
  iran_mask = df["program_str"].str.contains(
      "IRAN|IRGC|IFSR|IFCA|HRIT-IR", case=False
  )

  # Includes HKAA, HK-EO13936, CMIC, and country tags in remarks
  china_hk_mask = df["program_str"].str.contains(
      "HKAA|HK-EO13936|CMIC|UFLPA|CHINA", case=False
  ) | df["remarks_str"].str.contains(
      r"China|Hong Kong|\bPRC\b", case=False, regex=True
  )
  ...
  program_exposure = {
      "Russia / Ukraine Exposure": russia_ukr_mask.sum(),
      "Iran Exposure": iran_mask.sum(),
      "China / Hong Kong Exposure": china_hk_mask.sum(),
      "Counter-Terrorism (SDGT)": sdgt_mask.sum(),
      "Cyber-Related Sanctions": cyber_mask.sum(),
  }

  print("[3/3] Feature extraction complete.")

  return {
      "total_sdn_count": total_sdn_count,
      "type_counts": type_counts,
      "program_exposure": program_exposure,
      "raw_df": df,
  }
...
if __name__ == "__main__":
  try:
    ...
    print(
        "           L6-002: OFAC SANCTIONS & FINANCIAL WARFARE SUMMARY           "
    )
    print(
        "========================================================================="
    )
