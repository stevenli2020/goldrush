...
if 'CFTC_Contract_Market_Code' in df_disagg.columns:
    gold_df = df_disagg[df_disagg['CFTC_Contract_Market_Code'].astype(str).str.strip().str.zfill(6) == '088691'].copy()
else:
    gold_df = df_disagg[df_disagg['Market_and_Exchange_Names'].str.strip() == 'GOLD - COMMODITY EXCHANGE INC.'].copy()
date_parsed = False
for date_col in ['Report_Date_as_YYYY-MM-DD', 'Report_Date_as_MM_DD_YYYY', 'As_of_Date_In_Form_YYMMDD', 'As_of_Date_In_Form_YYYYMMDD']:
...
result_df = gold_df[['Parsed_Date', 'L10_002_Gold_Open_Interest', long_col, short_col, 'L10_001_Managed_Money_Net_Pos']].copy()
print('\n' + '=' * 50)
