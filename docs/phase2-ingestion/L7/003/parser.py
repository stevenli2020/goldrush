...
STALE_DAYS = 270
FIELDS = ["variable_id", "observation_date", "private_nonfinancial_credit_usd_billions", "credit_growth_yoy_pct", "unit", "aggregate_id", "source_name", "source_dataset_id", "source_series_key", "raw_path", "raw_sha256", "manifest_path", "manifest_sha256", "retrieved_at", "validation_status", "availability_status", "fallback_checked_at", "parser_version"]
MANIFEST_FIELDS = {"dataset_id", "series_key", "source_url", "retrieved_at", "raw_path", "size_bytes", "sha256", "http_status", "collector_version"}
...
def parse(raw_path: Path, manifest_path: Path, *, as_of: str | None = None, stale_days: int = STALE_DAYS) -> list[dict]:
    ...
    with raw_path.open(newline="", encoding="utf-8-sig") as handle:
        for row in csv.DictReader(handle):
            if not all(row.get(key) == value for key, value in {
                "FREQ": "Q", "BORROWERS_CTY": "5A", "TC_BORROWERS": "P",
                "TC_LENDERS": "A", "VALUATION": "M", "UNIT_TYPE": "USD", "UNIT_MULT": "9", "TC_ADJUST": "A",
            }.items()):
                continue
    ...
    for period in sorted(values):
        ...
        rows.append({
            ...
            "private_nonfinancial_credit_usd_billions": values[period], "credit_growth_yoy_pct": growth,
            "unit": "USD_billions", "aggregate_id": COVERAGE_ID,
            "source_name": "BIS Credit to the non-financial sector", "source_dataset_id": DATASET_ID,
            "source_series_key": SERIES_KEY, "raw_path": str(raw_path), "raw_sha256": raw_hash,
...
def validate_prior_row(row: dict) -> None:
    expected = {
        "variable_id": VARIABLE_ID, "unit": "USD_billions", "aggregate_id": COVERAGE_ID,
        "source_name": "BIS Credit to the non-financial sector", "source_dataset_id": DATASET_ID,
