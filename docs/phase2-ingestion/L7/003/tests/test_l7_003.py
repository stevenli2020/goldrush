...
def package(tmp_path, rows, *, dataset=parser.DATASET_ID, key=parser.SERIES_KEY, good_hash=True):
    ...
    manifest.write_text(json.dumps({
        "dataset_id": dataset, "series_key": key, "source_url": collector.SOURCE_URL,
        "retrieved_at": "2026-08-24T00:00:00+00:00", "raw_path": str(raw),
        "size_bytes": raw.stat().st_size, "sha256": digest if good_hash else "0" * 64,
        "http_status": 200, "collector_version": collector.COLLECTOR_VERSION,
    }), encoding="utf-8")
    return raw, manifest
...
def test_valid_parse_missing_growth_order_provenance_and_schema(tmp_path):
    ...
    assert records[1]["raw_sha256"] == hashlib.sha256(raw.read_bytes()).hexdigest()
    validate(records[0]); validate(records[1])
...
def test_malformed_nonpositive_and_incompatible_series_rejected(tmp_path, bad):
    ...
    with pytest.raises(ValueError): parser.parse(raw, manifest)


def test_conflicting_duplicate_and_hash_or_identifiers_rejected(tmp_path):
    raw, manifest = package(tmp_path, [row("2026-Q1", "100"), row("2026-Q1", "101")])
    with pytest.raises(ValueError, match="conflicting duplicate"): parser.parse(raw, manifest)
    raw, manifest = package(tmp_path / "hash", [row("2026-Q1", "100")], good_hash=False)
    with pytest.raises(ValueError, match="SHA-256"): parser.parse(raw, manifest)
    raw, manifest = package(tmp_path / "dataset", [row("2026-Q1", "100")], dataset="WRONG")
    with pytest.raises(ValueError, match="dataset or series"): parser.parse(raw, manifest)
    raw, manifest = package(tmp_path / "key", [row("2026-Q1", "100")], key="WRONG")
    with pytest.raises(ValueError, match="dataset or series"): parser.parse(raw, manifest)
...
def test_collector_preserves_bytes_and_manifest(tmp_path):
    data = (",".join(HEADER) + "\n").encode()
    class Response: status_code = 200; content = data
    class Session:
...
def test_manifest_retrieved_at_is_required_and_controlled(tmp_path, retrieved):
    ...
    if retrieved is None: metadata.pop("retrieved_at")
    else: metadata["retrieved_at"] = retrieved
    manifest.write_text(json.dumps(metadata))
    with pytest.raises(ValueError, match="retrieved_at"): parser.parse(raw, manifest)


def test_unit_multiplier_nine_is_required(tmp_path):
    raw, manifest = package(tmp_path, [row("2026-Q1", "100", UNIT_MULT="6")])
    with pytest.raises(ValueError, match="no BIS"): parser.parse(raw, manifest)
...
def test_malformed_manifest_falls_back_and_prior_identity_is_checked(tmp_path, monkeypatch):
    ...
    prior = tmp_path / "prior.csv"; parser.write_csv(parser.parse(raw, manifest, as_of="2026-08-24"), prior)
    metadata = json.loads(manifest.read_text()); metadata["retrieved_at"] = "bad"
    manifest.write_text(json.dumps(metadata))
    ...
    with output.open(newline="", encoding="utf-8") as handle: assert next(csv.DictReader(handle))["availability_status"] == "STALE"
    rows = list(csv.DictReader(prior.open(newline="", encoding="utf-8"))); rows[0]["source_series_key"] = "WRONG"
    with prior.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=parser.FIELDS); writer.writeheader(); writer.writerows(rows)
    with pytest.raises(ValueError, match="identity"): parser.carry_forward(prior)
...
def test_cli_fallback_blocked_and_recovery(tmp_path, monkeypatch):
    ...
    assert parser.main() == 0
    with output.open(newline="", encoding="utf-8") as handle: fallback = next(csv.DictReader(handle))
    assert fallback["availability_status"] == "STALE"; validate({**fallback, "private_nonfinancial_credit_usd_billions": float(fallback["private_nonfinancial_credit_usd_billions"]), "credit_growth_yoy_pct": float(fallback["credit_growth_yoy_pct"])})
    output.unlink()
