...
class L7001ParserTests(unittest.TestCase):
    ...
    def write_manifest(self, root, raw, *, series_id='WALCL', metadata=None):
        path = Path(root) / 'manifest.json'
        path.write_text(json.dumps({'series_id': series_id, 'retrieved_at': '2026-08-20T20:30:00+00:00'}), encoding='utf-8')
        return path

    def test_valid_weekly_parse_missing_marker_order_and_schema(self):
        with tempfile.TemporaryDirectory() as directory:
            raw = self.write_raw(directory, [{'date': '2026-08-19', 'value': '6745699'}, {'date': '2026-08-05', 'value': '6748567'}, {'date': '2026-08-12', 'value': '.'}])
    ...
    def test_bad_value_and_date_are_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            for observation in ({'date': 'bad', 'value': '6745699'}, {'date': '2026-08-19', 'value': 'nan'}, {'date': '2026-08-19', 'value': '-1'}):
    ...
    def test_duplicate_dates_dedupe_or_reject_conflicts(self):
        with tempfile.TemporaryDirectory() as directory:
            raw = self.write_raw(directory, [{'date': '2026-08-19', 'value': '6745699'}, {'date': '2026-08-19', 'value': '6745699'}])
            manifest = self.write_manifest(directory, raw)
            self.assertEqual(len(parse_observations(raw, manifest)), 1)
            raw = self.write_raw(directory, [{'date': '2026-08-19', 'value': '6745699'}, {'date': '2026-08-19', 'value': '6745700'}])
            manifest = self.write_manifest(directory, raw)
            with self.assertRaises(ValueError):
                parse_observations(raw, manifest)
