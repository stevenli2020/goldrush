...
OUTPUT_FIELDS = [
    "variable_id", "observation_date", "sofr_percent", "effr_percent",
    "repo_funding_stress_bps", "unit", "observation_definition",
    "source_name", "source_release", "source_attribution", "sofr_series_id",
    "effr_series_id", "sofr_raw_file_path", "effr_raw_file_path",
    "sofr_manifest_file_path", "effr_manifest_file_path", "sofr_raw_sha256",
    "effr_raw_sha256", "sofr_retrieved_at", "effr_retrieved_at",
...
def load_series(raw_path: Path, manifest_path: Path, expected_series: str) -> tuple[dict[str, float], dict[str, Any], str]:
    ...
    if manifest.get("sha256") != digest:
        raise ValueError(f"{expected_series} raw file hash does not match manifest")
    ...
    return values, manifest, digest


def parse_observations(
    sofr_raw: Path, sofr_manifest: Path, effr_raw: Path, effr_manifest: Path,
    *, stale_after_days: int = 5, today: date | None = None,
) -> list[dict[str, Any]]:
    sofr, sm, shash = load_series(sofr_raw, sofr_manifest, SOFR)
    effr, em, ehash = load_series(effr_raw, effr_manifest, EFFR)
    ...
    for date_text in overlap:
        ...
        # Broad bounds retain genuine stress while catching malformed inputs.
        validation = "PASS" if -500.0 <= spread <= 2000.0 and -10.0 <= sofr[date_text] <= 50.0 and -10.0 <= effr[date_text] <= 50.0 else "FLAG"
        rows.append({
            "variable_id": VARIABLE_ID, "observation_date": date_text,
            "sofr_percent": sofr[date_text], "effr_percent": effr[date_text],
            "repo_funding_stress_bps": spread, "unit": "basis_points",
            "observation_definition": "SOFR minus EFFR, multiplied by 100; secured Treasury repo funding relative to effective federal funds",
            "source_name": "FRED", "source_release": "Federal Reserve Bank of New York Reference Rates",
            "source_attribution": "Federal Reserve Bank of New York via FRED",
            "sofr_series_id": SOFR, "effr_series_id": EFFR,
            "sofr_raw_file_path": str(sofr_raw), "effr_raw_file_path": str(effr_raw),
            "sofr_manifest_file_path": str(sofr_manifest), "effr_manifest_file_path": str(effr_manifest),
            "sofr_raw_sha256": shash, "effr_raw_sha256": ehash,
            "sofr_retrieved_at": sm["retrieved_at"], "effr_retrieved_at": em["retrieved_at"],
            "validation_status": validation,
            "availability_status": "STALE" if (as_of - date.fromisoformat(date_text)).days > stale_after_days else "AVAILABLE",
