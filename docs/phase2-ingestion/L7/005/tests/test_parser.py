...
class L7005Tests(unittest.TestCase):
    ...
    def manifest(self, root, name, raw, series):
        ...
        path.write_text(json.dumps({'series_id': series, 'retrieved_at': '2026-08-24T00:00:00+00:00'}))
        return path

    def pair(self, root, so, ef):
        sr = self.raw(root, 'sofr', so)
        er = self.raw(root, 'effr', ef)
    ...
    def test_effr_provenance_failures_are_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            em.write_text(json.dumps({'series_id': 'WALCL', 'retrieved_at': '2026-08-24T00:00:00+00:00'}))
            with self.assertRaises(ValueError):
                parse_observations(sr, sm, er, em)
            em.write_text(json.dumps({'series_id': 'EFFR'}))
            with self.assertRaises(ValueError):
                parse_observations(sr, sm, er, em)
            em.write_text(json.dumps({'series_id': 'EFFR', 'retrieved_at': '2026-08-24T00:00:00+00:00'}))
            sm.write_text(json.dumps({'series_id': 'SOFR'}))
            with self.assertRaises(ValueError):
                parse_observations(sr, sm, er, em)
    ...
    def test_cli_fallback_blocked_and_recovery(self):
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            out = root / 'out.csv'
            cmd = [sys.executable, str(Path(__file__).parents[1] / 'parser.py'), '--output', str(out)]
            ...
            bad = self.raw(d, 'bad', [{'date': 'bad', 'value': '1'}])
            bm = self.manifest(d, 'bad', bad, 'SOFR')
            ...
            for field in ('sofr_retrieved_at', 'effr_retrieved_at', 'sofr_raw_file_path', 'effr_raw_file_path', 'sofr_manifest_file_path', 'effr_manifest_file_path'):
