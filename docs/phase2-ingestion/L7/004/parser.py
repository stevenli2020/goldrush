"""Parse preserved FRED BAMLH0A0HYM2 observations for L7-004."""
...
OUTPUT_FIELDS = ['variable_id', 'observation_date', 'high_yield_oas_pct', 'unit', 'observation_definition', 'source_name', 'source_series_id', 'source_release', 'source_attribution', 'raw_file_path', 'manifest_file_path', 'retrieved_at', 'validation_status', 'availability_status', 'parser_version']
...
def parse_observations(raw_path: Path, manifest_path: Path, *, stale_after_days: int=7, today: date | None=None) -> list[dict[str, Any]]:
    ...
    for item in payload['observations']:
        ...
        if not math.isfinite(value):
            raise ValueError(f'non-finite spread value: {value_text}')
    ...
    for date_text in sorted(observed):
        ...
        rows.append({'variable_id': VARIABLE_ID, 'observation_date': date_text, 'high_yield_oas_pct': value, 'unit': 'percentage_points', 'observation_definition': 'ICE BofA US High Yield Index option-adjusted spread at daily close', 'source_name': 'FRED', 'source_series_id': SERIES_ID, 'source_release': 'ICE BofA Indices', 'source_attribution': 'ICE Data Indices, LLC via FRED', 'raw_file_path': str(raw_path), 'manifest_file_path': str(manifest_path), 'retrieved_at': retrieved_at, 'validation_status': 'PASS' if 0.5 <= value <= 30 else 'FLAG', 'availability_status': 'STALE' if (as_of - date.fromisoformat(date_text)).days > stale_after_days else 'AVAILABLE', 'parser_version': PARSER_VERSION})
    return rows

def carry_forward(prior_path: Path, *, retrieved_at: str | None=None) -> list[dict[str, Any]]:
    if not prior_path.exists():
        raise FileNotFoundError('no prior L7-004 observation is available')
    with prior_path.open(newline='', encoding='utf-8') as handle:
        rows = list(csv.DictReader(handle))
    valid = [row for row in rows if row.get('validation_status') == 'PASS']
    if not valid:
        raise ValueError('prior L7-004 output contains no valid observation')
    latest = max(valid, key=lambda row: row['observation_date']).copy()
    latest['high_yield_oas_pct'] = float(latest['high_yield_oas_pct'])
...
def main(argv: list[str] | None=None) -> int:
    cli = argparse.ArgumentParser(description='Parse FRED BAMLH0A0HYM2 observations for L7-004')
    cli.add_argument('--raw', type=Path)
    cli.add_argument('--manifest', type=Path)
    cli.add_argument('--prior', type=Path)
    cli.add_argument('--output', type=Path, default=Path('data/processed/L7_004_observations.csv'))
    cli.add_argument('--stale-after-days', type=int, default=7)
