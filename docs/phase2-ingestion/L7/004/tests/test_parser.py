...
class L7004ParserTests(unittest.TestCase):
    ...
    def write_manifest(self, root, raw, *, series_id="BAMLH0A0HYM2", sha256=None):
        path = Path(root) / "manifest.json"
        path.write_text(json.dumps({
            "series_id": series_id,
            "sha256": sha256 or hashlib.sha256(raw.read_bytes()).hexdigest(),
            "retrieved_at": "2026-08-23T20:30:00+00:00",
        }), encoding="utf-8")
        return path
    ...
    def test_valid_parse_missing_marker_order_units_weekend_and_schema(self):
        with tempfile.TemporaryDirectory() as directory:
            raw = self.write_raw(directory, [
                {"date": "2026-08-21", "value": "3.06"},
                {"date": "2026-08-19", "value": "3.12"},
    ...
    def test_bad_date_nonfinite_rejected_and_finite_outlier_flagged(self):
        with tempfile.TemporaryDirectory() as directory:
            for observation in ({"date": "bad", "value": "3.1"},
                                {"date": "2026-08-21", "value": "nan"}):
    ...
    def test_wrong_series_and_hash_mismatch_are_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(ValueError):
                parse_observations(raw, self.write_manifest(directory, raw, series_id="WALCL"))
            with self.assertRaises(ValueError):
                parse_observations(raw, self.write_manifest(directory, raw, sha256="0" * 64))
