...
def fetch_dataset(endpoint: str, query: dict[str, Any], raw_dir: Path, manifest_dir: Path, *, session: Any | None=None, timeout: int=30, max_retries: int=2) -> dict[str, Any]:
    ...
    while page_number <= total_pages:
        ...
        for attempt in range(max_retries + 1):
            try:
                response = client.get(endpoint, params=params, timeout=timeout, headers={'User-Agent': USER_AGENT, 'Accept': 'application/json'})
    ...
    manifest = {'endpoint': endpoint, 'query': base_query, 'retrieved_at': retrieved_at.isoformat(), 'raw_paths': raw_paths, 'page_count': len(raw_paths), 'record_count': record_count, 'byte_size': sum((len(content) for content in contents)), 'client_version': CLIENT_VERSION}
