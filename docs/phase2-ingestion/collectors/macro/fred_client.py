...
def fetch_series(
    ...
    *,
    ...
    changed = force or not previous or previous.get("sha256") != digest
    ...
    record = {
        "series_id": series_id,
        "request_url": FRED_OBSERVATIONS_URL,
        "request_params": {key: value for key, value in params.items() if key != "api_key"},
    ...
    (manifest_dir / f"{series_id}-{stamp}.json").write_text(
