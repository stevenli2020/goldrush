...
DISPATCH = {
    "l0_009": (ROOT / "L0" / "009" / "parser.py", ROOT / "data" / "cme" / "processed" / "section62_normalized.csv", ROOT / "L0" / "009" / "data" / "raw" / "sofr3m.csv", ROOT / "L0" / "009" / "data" / "processed" / "L0_009_observations.csv"),
    "l10_002": (ROOT / "L10" / "002" / "parser.py", ROOT / "L10" / "002" / "data" / "raw" / "section02b_normalized.csv", None, ROOT / "L10" / "002" / "data" / "processed" / "L10_002_observations.csv"),
    "l3_001": (ROOT / "L3" / "001" / "parser.py", ROOT / "L3" / "001" / "data" / "raw" / "fed_funds_strip.csv", None, ROOT / "L3" / "001" / "data" / "processed" / "L3_001_observations.csv"),
}
...
def latest_manifests() -> list[dict]:
    ...
    for path in MANIFESTS.glob("*.json"):
        ...
        if record.get("target") and (record["target"] not in latest or record.get("retrieved_at", "") > latest[record["target"]].get("retrieved_at", "")): latest[record["target"]] = record
    return list(latest.values())

def run(command: list[str], verbose: bool) -> tuple[int, str, str]:
    if verbose: print("[debug] " + " ".join(command))
    result = subprocess.run(command, cwd=ROOT, text=True, capture_output=True)
    if verbose and result.stdout.strip(): print(result.stdout.strip())
...
def dispatch_normalized(results: list[dict], verbose: bool) -> None:
    for name, (parser, primary, secondary, output) in DISPATCH.items():
        ...
        if name == "l0_009": command += ["--cme", str(primary), "--sofr", str(secondary)]
        else: command += ["--input", str(primary)]
...
def main(argv=None) -> int:
    ...
    for manifest in manifests:
        ...
        if code != 0:
            if "30D FED FD FUT section not found" in err:
                results.append({"target": target, "status": "INSPECTED_UNUSED", "reason": "required marker not present; raw file retained", "sha256": manifest["sha256"]})
            else:
    ...
    if candidates:
        ...
        code, out, err = run([sys.executable, str(PARSER), str(normalized), "--retrieved-at", manifest["retrieved_at"], "--sha256", manifest["sha256"], "--output", str(OUTPUT)], args.verbose)
        results.append({"target": target, "status": "PASS" if code == 0 else "FAIL", "stage": "parse", "selected": True, "sha256": manifest["sha256"], "stdout": out[-1000:], "stderr": err[-1000:]})
        for other_manifest, _, other_target in candidates[1:]:
            results.append({"target": other_target, "status": "INSPECTED_UNUSED", "reason": f"another section selected ({target}); raw file retained", "sha256": other_manifest["sha256"]})
