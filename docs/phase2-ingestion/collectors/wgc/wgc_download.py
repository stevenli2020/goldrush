...
def filename_from_response(response, url: str) -> str:
    ...
    match = re.search(r"filename\*=UTF-8''([^;]+)|filename=\"?([^;\"]+)", header, re.I)
...
def find_download_url(html: str, pattern: str, page_url: str) -> str:
    ...
    if not match: raise RuntimeError("no matching workbook link found")
...
def download_target(session, name, target, roots):
    ...
    if page.status_code != 200: raise RuntimeError(f"page HTTP status {page.status_code}")
    ...
    if changed:
        raw_path.parent.mkdir(parents=True, exist_ok=True); raw_path.write_bytes(content)
    record = {"target": name, "page_url": target["page_url"], "download_url": url, "filename": filename,
              "raw_path": str(raw_path), "size_bytes": len(content), "sha256": digest,
              "downloaded_at": now.isoformat(), "http_status": response.status_code,
              "content_type": response.headers.get("Content-Type", ""), "changed": changed}
    roots["manifests"].mkdir(parents=True, exist_ok=True)
    stamp = now.strftime("%Y%m%dT%H%M%SZ")
    (roots["manifests"] / f"{name}-{stamp}.json").write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
    return record

def main(argv=None):
    cli = argparse.ArgumentParser(); cli.add_argument("--config", type=Path, default=Path(__file__).with_name("config.yaml")); args = cli.parse_args(argv)
    config_path, config = args.config.resolve(), load_config(args.config.resolve()); base = config_path.parent
    roots = {k: (base / config["collector"][v]).resolve() for k, v in {"raw":"raw_root", "manifests":"manifest_root", "logs":"log_root"}.items()}
    session = requests.Session(); cookie_path = (base / config["collector"]["cookie_jar"]).resolve()
    for cookie in load_cookies(cookie_path):
        if cookie.get("name") and cookie.get("value"): session.cookies.set(cookie["name"], cookie["value"], domain=cookie.get("domain", ".gold.org"))
    results = []
    for name, target in config["targets"].items():
        try:
            result = download_target(session, name, target, roots); results.append(result); print(f"OK {name}: {result['filename']} ({'changed' if result['changed'] else 'unchanged'})")
        except Exception as exc:
            results.append({"target": name, "error": str(exc)}); print(f"ERROR {name}: {exc}", file=sys.stderr)
    roots["logs"].mkdir(parents=True, exist_ok=True); stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
