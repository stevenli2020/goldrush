...
def latest_manifests(root: Path) -> list[dict]:
    ...
    for path in root.glob("*.json"):
        try:
            ...
            continue
        if "target" in record and "sha256" in record:
...
def run_parser(mapping: dict, raw_path: Path, project_root: Path, manifest: dict, verbose: bool = False) -> dict:
    ...
    replacements = {"{input}": str(raw_path), "{download_date}": manifest.get("downloaded_at", "")[:10],
    ...
    if verbose: print(f"[extract] running: {' '.join(command)}", flush=True)
    ...
    if verbose:
        ...
        if completed.stdout.strip(): print(f"[extract] stdout: {completed.stdout.strip()[-1000:]}", flush=True)
        if completed.stderr.strip(): print(f"[extract] stderr: {completed.stderr.strip()[-1000:]}", flush=True)
...
def main(argv=None) -> int:
    ...
    if args.verbose: print(f"[extract] loaded {len(manifests)} latest manifest(s) from {manifest_root}", flush=True)
    for manifest in manifests:
        target = manifest["target"]
        if args.verbose: print(f"[extract] target={target} changed={manifest.get('changed', True)} sha256={manifest.get('sha256')}", flush=True)
        mapping = mappings.get(target)
        if not mapping:
            if args.verbose: print(f"[extract] {target}: no parser mapping; skipped", flush=True)
        ...
        if not args.force and not manifest.get("changed", True):
            if args.verbose: print(f"[extract] {target}: unchanged; skipped (use --force to rerun)", flush=True)
            results.append({"target": target, "status": "UNCHANGED", "sha256": manifest["sha256"]})
            continue
        raw_path = Path(manifest["raw_path"])
        if not raw_path.is_absolute(): raw_path = (project_root / raw_path).resolve()
        try:
            ...
            result.update({"target": target, "status": "PASS" if result["returncode"] == 0 else "FAIL", "sha256": manifest["sha256"]})
        except OSError as exc:
            result = {"target": target, "status": "FAIL", "error": str(exc), "sha256": manifest["sha256"]}
    ...
    if args.verbose: print(f"[extract] run log: {log_root / f'wgc-extract-{stamp}.json'}", flush=True)
