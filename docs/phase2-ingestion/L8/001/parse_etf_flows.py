...
def parse_file(path: Path, publication_date: str, download_date: str) -> pd.DataFrame:
    ...
    if header is None: raise ValueError("Fund flows by month date header not found")
    ...
    if tonnes_col is None: raise ValueError("Aggregate tonnes column not found")
    digest = hashlib.sha256(path.read_bytes()).hexdigest(); records = []
    for _, row in raw.iloc[header + 1:].iterrows():
        if not pd.notna(row.iloc[date_col]) or not pd.notna(row.iloc[tonnes_col]): continue
        date = pd.to_datetime(row.iloc[date_col], errors="coerce")
        value = pd.to_numeric(row.iloc[tonnes_col], errors="coerce")
        if pd.isna(date) or pd.isna(value): continue
        records.append({"variable_id":"L8-001", "observation_date":date.strftime("%Y-%m-%d"),
            "etf_flow_tonnes":float(value), "unit":"metric_tonnes", "source_file":path.name,
    ...
    if not records: raise ValueError("No ETF flow records found")
...
def main(argv=None):
    ...
    df=parse_file(a.input,a.publication_date,a.download_date); a.output.parent.mkdir(parents=True,exist_ok=True); df.to_csv(a.output,index=False); print(f"ETF flow records parsed: {len(df)}")
