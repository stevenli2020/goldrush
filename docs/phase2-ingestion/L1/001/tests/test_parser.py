...
spec = importlib.util.spec_from_file_location('l1_001_parser', Path(__file__).parents[1] / 'parser.py')
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
parse_observations, write_csv = (module.parse_observations, module.write_csv)

class L1001ParserTests(unittest.TestCase):

    def test_parse_valid_observations_and_stale_status(self):
        payload = {'observations': [{'date': '2026-08-20', 'value': '1.75'}, {'date': '2020-01-02', 'value': '0.25'}]}
        with tempfile.TemporaryDirectory() as directory:
            raw = Path(directory) / 'DFII10.json'
            raw.write_text(json.dumps(payload), encoding='utf-8')
            rows = parse_observations(raw, retrieved_at='2026-08-21T00:00:00+00:00')
            self.assertEqual(rows[0]['source_series_id'], 'DFII10')
    ...
    def test_fred_missing_value_is_skipped(self):
        payload = {'observations': [{'date': '2026-08-19', 'value': '.'}, {'date': '2026-08-20', 'value': '1.75'}]}
        with tempfile.TemporaryDirectory() as directory:
            raw = Path(directory) / 'DFII10.json'
            raw.write_text(json.dumps(payload), encoding='utf-8')
            rows = parse_observations(raw)
