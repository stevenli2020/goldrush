...
def download(output_dir: Path, *, url: str=SOURCE_URL, session=None) -> dict:
    ...
    metadata = hashlib.sha256(content).hexdigest()
    output_dir.mkdir(parents=True, exist_ok=True)
    raw_path = output_dir / f'feds200805-{stamp}.csv'
    raw_path.write_bytes(content)
    record = {'source_url': url, 'raw_path': str(raw_path), 'size_bytes': len(content), 'sha256': metadata, 'downloaded_at': now.isoformat(), 'http_status': response.status_code, 'parser_version': VERSION}
