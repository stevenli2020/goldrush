...
FIELDS = ['variable_id', 'observation_date', 'value', 'unit', 'forward_2y1y', 'forward_3y2y', 'forward_5y2y', 'forward_7y3y', 'forward_10y10y', 'source_name', 'source_url', 'raw_file_path', 'source_retrieved_at', 'formula_version', 'parser_version', 'is_revised', 'revision_reason', 'validation_status', 'availability_status']
...
def read_rows(path: Path):
    ...
    if header_index is None:
        raise ValueError('GS&W CSV Date header not found')
    headers = lines[header_index]
    required = ['Date', *INPUTS]
    if any((name not in headers for name in required)):
        raise ValueError('GS&W CSV missing required TIPSY columns')
    ...
    if previous_output and previous_output.exists():
        with previous_output.open(newline='', encoding='utf-8') as handle:
            for row in csv.DictReader(handle):
                prior[row.get('observation_date', '')] = row
    output = []
    for item in source_rows:
        if any((item[name] in ('', '.', 'NA', 'N/A') for name in INPUTS)):
            continue
        try:
            date = datetime.strptime(item['Date'], '%Y-%m-%d').date()
            values = {name: float(item[name]) for name in INPUTS}
        except (TypeError, ValueError) as exc:
            raise ValueError(f'malformed GS&W row: {item}') from exc
        components = calculate_components(values)
        value = sum(components.values()) / len(components)
        previous = prior.get(item['Date'])
        revised = bool(previous and previous.get('value') and (abs(float(previous['value']) - value) > 1e-12))
        output.append({'variable_id': VARIABLE_ID, 'observation_date': item['Date'], 'value': value, **components, 'unit': 'percent', 'source_name': 'Federal Reserve GS&W', 'source_url': 'https://www.federalreserve.gov/data/yield-curve-tables/feds200805.csv', 'raw_file_path': str(raw_path), 'source_retrieved_at': source_retrieved_at, 'formula_version': FORMULA_VERSION, 'parser_version': PARSER_VERSION, 'is_revised': revised, 'revision_reason': 'Source revision changed derived value' if revised else None, 'validation_status': 'PASS' if -10 <= value <= 20 else 'FLAG', 'availability_status': 'STALE' if (now.date() - date).days > stale_after_days else 'AVAILABLE'})
    if not output:
        raise ValueError('no complete six-input observations found')
    return output
...
if __name__ == '__main__':
    ...
    ap.add_argument('--output', type=Path, default=Path('data/processed/L1_003_observations.csv'))
    ap.add_argument('--previous-output', type=Path)
    args = ap.parse_args()
    rows = parse_source(args.raw, source_retrieved_at=args.source_retrieved_at, previous_output=args.previous_output)
