...
FIELDS = ['variable_id', 'observation_date', 'value', 'unit', 'source_name', 'input_5y_series_id', 'input_10y_series_id', 'input_5y_raw_path', 'input_10y_raw_path', 'input_5y_retrieved_at', 'input_10y_retrieved_at', 'formula_version', 'parser_version', 'validation_status', 'availability_status']
...
def parse_observations(dfii5_path: Path, dfii10_path: Path, *, dfii5_retrieved_at: str, dfii10_retrieved_at: str, stale_after_days: int=7) -> list[dict[str, object]]:
    five_year, = load_series(dfii5_path)
    ten_year, = load_series(dfii10_path)
    ...
    for date_text in sorted(set(five_year) & set(ten_year)):
        ...
        rows.append({'variable_id': VARIABLE_ID, 'observation_date': date_text, 'value': value, 'unit': 'percent', 'source_name': 'FRED-derived', 'input_5y_series_id': SERIES_5Y, 'input_10y_series_id': SERIES_10Y, 'input_5y_raw_path': str(dfii5_path), 'input_10y_raw_path': str(dfii10_path), 'input_5y_retrieved_at': dfii5_retrieved_at, 'input_10y_retrieved_at': dfii10_retrieved_at, 'formula_version': FORMULA_VERSION, 'parser_version': PARSER_VERSION, 'validation_status': 'PASS' if -10 <= value <= 20 else 'FLAG', 'availability_status': 'STALE' if (now.date() - date).days > stale_after_days else 'AVAILABLE'})
    if not rows:
