...
CONTRACT_RE = re.compile(r"\b([A-Z]{3})(\d{2})\b")
SETTLE_RE = re.compile(r"([0-9]{2,3}\.[0-9]{4})\s*\(")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(); p.add_argument("pdf_path", type=Path); p.add_argument("--observation-date", required=True); p.add_argument("--output", type=Path, required=True)
    a = p.parse_args(); print({"rows": extract_pdf(a.pdf_path, date.fromisoformat(a.observation_date), a.output), "output": str(a.output)})
