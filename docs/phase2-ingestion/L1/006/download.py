"""Preserve the public CME interest-rate bulletin and write a source metadata manifest."""
...
def download(out_dir: Path, url: str=URL) -> dict:
    ...
    sha = None
    manifest = {'variable_id': 'L1-006', 'source_url': url, 'raw_path': str(raw), 'size_bytes': len(data), 'retrieved_at': retrieved.isoformat(), 'http_status': status, 'content_type': content_type}
    raw.with_suffix('.manifest.json').write_text(json.dumps(manifest, indent=2), encoding='utf-8')
    return manifest
if __name__ == '__main__':
    p = argparse.ArgumentParser()
    p.add_argument('--out-dir', type=Path, default=Path(__file__).parent / 'data/raw')
