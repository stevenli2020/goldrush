...
CONTRACT_RE = re.compile('^ZQ[A-Z]\\d{2}$')

def parse(path: Path, retrieved_at: str, source_pdf_metadata: str='', source_url: str='https://www.cmegroup.com/daily_bulletin/current/Section10_Interest_Rate_Futures_Continued.pdf') -> list[dict]:
    candidates = []
    with path.open(newline='', encoding='utf-8') as f:
        ...
        for row in reader:
            try:
                ...
                if not obs or not expiry or settle <= 0:
                    raise ValueError
                value = 100.0 - settle
                ...
                candidates.append({'variable_id': 'L1-006', 'observation_date': obs.isoformat(), 'value': value, 'unit': 'percent_per_annum', 'source_name': 'CME 30-Day Fed Funds futures', 'source_series_id': contract, 'settlement_price': settle, 'expiry_date': expiry.isoformat(), 'source_url': source_url, 'retrieved_at': retrieved_at, 'formula_version': '1.0.0', 'parser_version': PARSER_VERSION, 'validation_status': validation, 'availability_status': availability})
...
if __name__ == '__main__':
    ...
    a = p.parse_args()
    out = parse(a.csv_path, a.retrieved_at)
