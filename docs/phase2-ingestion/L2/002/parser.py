"""Parse preserved FRED DTWEXBGS observations for L2-002."""
...
OUTPUT_FIELDS = ['variable_id', 'observation_date', 'index_value', 'unit', 'index_definition', 'source_name', 'source_series_id', 'source_release', 'raw_file_path', 'retrieved_at', 'validation_status', 'availability_status', 'parser_version']
...
def parse_observations(raw_path: Path, *, manifest_path: Path | None=None, retrieved_at: str | None=None, stale_after_days: int=10) -> list[dict[str, Any]]:
    ...
    for date_text in sorted(observed):
        ...
        status = 'FLAG' if not 50 <= value <= 200 else 'PASS'
        availability = 'STALE' if age_days > stale_after_days else 'AVAILABLE'
        rows.append({'variable_id': VARIABLE_ID, 'observation_date': date_text, 'index_value': value, 'unit': 'index', 'index_definition': 'Index Jan 2006=100, not seasonally adjusted', 'source_name': 'FRED', 'source_series_id': SERIES_ID, 'source_release': 'H.10 Foreign Exchange Rates', 'raw_file_path': str(raw_path), 'retrieved_at': retrieval, 'validation_status': status, 'availability_status': availability, 'parser_version': PARSER_VERSION})
    return rows

def carry_forward(prior_path: Path, *, retrieved_at: str | None=None) -> list[dict[str, Any]]:
    if not prior_path.exists():
        raise FileNotFoundError('no prior L2-002 observation is available')
    with prior_path.open(newline='', encoding='utf-8') as handle:
        rows = list(csv.DictReader(handle))
    valid = [row for row in rows if row.get('validation_status') == 'PASS']
    if not valid:
        raise ValueError('prior L2-002 output contains no valid observation')
...
def main(argv: list[str] | None=None) -> int:
    cli = argparse.ArgumentParser(description='Parse FRED DTWEXBGS observations for L2-002')
    cli.add_argument('--raw', type=Path)
    cli.add_argument('--manifest', type=Path)
    cli.add_argument('--prior', type=Path)
    cli.add_argument('--output', type=Path, default=Path('data/processed/L2_002_observations.csv'))
