...
class L2002ParserTests(unittest.TestCase):
    ...
    def write_manifest(self, directory, raw_path, series_id="DTWEXBGS"):
        ...
        path.write_text(json.dumps({"series_id": series_id, "sha256": digest,
    ...
    def test_valid_missing_and_ordering(self):
        with tempfile.TemporaryDirectory() as directory:
            raw = self.write_raw(directory, {"observations": [
                {"date": "2026-01-03", "value": "119.2"},
                {"date": "2026-01-02", "value": "."},
                {"date": "2026-01-01", "value": "118.9"},
            ]})
    ...
    def test_cli_writes_blocked_status_without_prior(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            valid_raw = self.write_raw(directory, {"observations": [
                {"date": "2026-08-14", "value": "118.9028"},
            ]})
            recovered = subprocess.run(
                [sys.executable, str(Path(__file__).parents[1] / "parser.py"),
                 "--raw", str(valid_raw), "--output", str(output)],
                capture_output=True, text=True, check=False,
            )
