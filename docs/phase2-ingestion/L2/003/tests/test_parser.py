...
class L2003ParserTests(unittest.TestCase):
    ...
    def write_manifest(self, directory, raw_path, series_id="DEXCHUS"):
        ...
        path.write_text(json.dumps({"series_id": series_id,
                                    "sha256": hashlib.sha256(raw_path.read_bytes()).hexdigest(),
                                    "retrieved_at": "2026-08-23T00:00:00+00:00"}), encoding="utf-8")
        return path

    def test_quote_direction_missing_and_ordering(self):
        with tempfile.TemporaryDirectory() as directory:
            raw = self.write_raw(directory, {"observations": [
                {"date": "2026-01-03", "value": "6.8"},
                {"date": "2026-01-02", "value": "."},
                {"date": "2026-01-01", "value": "6.9"},
            ]})
    ...
    def test_invalid_duplicate_series_and_numeric_fail(self):
        with tempfile.TemporaryDirectory() as directory:
            duplicate = self.write_raw(directory, {"observations": [
                {"date": "2026-01-01", "value": "6.9"},
    ...
    def test_manifest_hash_freshness_and_range(self):
        with tempfile.TemporaryDirectory() as directory:
            raw = self.write_raw(directory, {"observations": [{"date": "2026-01-01", "value": "10"}]})
            manifest = self.write_manifest(directory, raw)
            rows = parse_observations(raw, manifest_path=manifest, stale_after_days=1)
            self.assertEqual(rows[0]["raw_sha256"], hashlib.sha256(raw.read_bytes()).hexdigest())
    ...
    def test_cli_prior_fallback_writes_schema_valid_stale_row(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            raw_hash = "a" * 64
            fields = [
                "variable_id", "observation_date", "usd_cny", "unit", "source_name",
                "source_series_id", "source_release", "raw_file_path", "manifest_path",
                "raw_sha256", "retrieved_at", "validation_status", "availability_status",
                "parser_version",
            ]
            row = {
                "variable_id": "L2-003", "observation_date": "2026-08-14", "usd_cny": "6.7412",
                "unit": "cny_per_usd", "source_name": "FRED / Federal Reserve Board",
                "source_series_id": "DEXCHUS", "source_release": "H.10 Foreign Exchange Rates",
                "raw_file_path": "raw.json", "manifest_path": "manifest.json", "raw_sha256": raw_hash,
                "retrieved_at": "2026-08-23T00:00:00+00:00", "validation_status": "PASS",
                "availability_status": "AVAILABLE", "parser_version": "0.1.0",
            }
            with prior.open("w", newline="", encoding="utf-8") as handle:
            ...
            result = subprocess.run(
                [sys.executable, script, "--raw", str(root / "missing.json"), "--prior", str(prior), "--output", str(output)],
