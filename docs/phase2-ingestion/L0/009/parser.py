...
def parse(cme_csv: Path, sofr_csv: Path) -> list[dict]:
    ...
    for row in cme:
        ...
        rows.append({'variable_id': 'L0-009', 'observation_date': obs.isoformat(), 'value': value, 'unit': 'percent_per_annum', 'forward_rate_percent': forward, 'sofr3m_percent': sofr[row['observation_date']], 'validation_status': 'PASS' if abs(value) <= 20 else 'FLAG', 'availability_status': 'AVAILABLE'})
    if not rows:
