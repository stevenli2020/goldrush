...
def extract_rows_from_text(text: str, observation_date: date) -> list[dict]:
    marker = re.search(r"GC FUT\s+COMEX GOLD FUTURES", text)
    if marker is None:
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(); p.add_argument("--pdf", type=Path, required=True); p.add_argument("--observation-date", required=True); p.add_argument("--output", type=Path, required=True)
    args = p.parse_args(); print(f"Wrote {extract_pdf(args.pdf, date.fromisoformat(args.observation_date), args.output)} row(s)")
