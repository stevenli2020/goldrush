...
def test_parse_normal_and_anomalies(parser, sample_csv):
    ...
    assert rec1['holdings_tonnes'] == 3100.5
...
def test_revision_detection(parser, tmp_path):
    ...
    prior = {('2026-08-01', 'GLOBAL'): 3100.5}
    ...
    assert rec['revision_metadata']['prior_value'] == 3100.5

def test_stale_fallback(parser):
    base_record = {'observation_date': '2026-08-01', 'holdings_tonnes': 3100.5, 'unit': 'metric_tonnes', 'region': 'GLOBAL', 'source_citation': 'World Gold Council', 'source_file': 'data.csv', 'publication_date': '2026-08-01', 'download_date': '2026-08-01', 'ingestion_timestamp': '2026-08-01T00:00:00Z', 'parser_version': '1.0.0', 'revision_metadata': {'is_revision': False, 'prior_value': None, 'revision_reason': None}, 'validation_status': 'PASS', 'availability_status': 'AVAILABLE'}
    fallback = parser.generate_stale_fallback(base_record, '2026-08-02')
    assert fallback['observation_date'] == '2026-08-02'
    assert fallback['holdings_tonnes'] == 3100.5
