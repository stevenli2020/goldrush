...
ANNUAL_PATTERN = re.compile('^\\d{4}$')
QUARTERLY_PATTERN = re.compile("^Q([1-4])'(\\d{2})$")
...
def metadata_of_file(path: Path) -> str:
...
def extract_gold_balance(wb: openpyxl.Workbook, cfg: dict, _unused: dict) -> dict:
    ...
    t = sheet_cfg['targets']
    rows = {'total_bar_and_coin_tonnes': get_row_values(ws, t['total_bar_and_coin']['row']), 'bar_demand_tonnes': get_row_values(ws, t['bars']['row']), 'official_coin_demand_tonnes': get_row_values(ws, t['official_coins']['row']), 'medals_imitation_coin_tonnes': get_row_values(ws, t['medals_imitation_coins']['row'])}
    result = {}
...
def extract_bar_and_coin_sheet(wb: openpyxl.Workbook, cfg: dict) -> dict:
    ...
    return {label: {'named_country_total_tonnes': row_named[col_idx], 'other_and_stock_change_tonnes': row_other[col_idx], 'world_total_bar_and_coin_sheet_tonnes': row_world[col_idx]} for label, col_idx in cols.items()}
...
def validate_record(rec: dict, cfg: dict, logger: logging.Logger) -> tuple:
    ...
    if ptype == 'annual':
        ...
        if all((v is not None for v in [bar, coins, medals])):
            ...
            if diff_pct > vcfg['subcomponent_fail_threshold_pct']:
                return ('FAIL', f"Sub-component sum {comp_sum:.5f} differs from total {total:.5f} by {diff_pct:.2f}% (FAIL threshold {vcfg['subcomponent_fail_threshold_pct']}%)")
    ...
    if world is not None and total:
        ...
        if diff_pct > vcfg['sheet_reconciliation_tolerance_pct']:
            notes.append(f'Bar and Coin sheet world_total={world:.5f} differs from Gold Balance total={total:.5f} by {diff_pct:.4f}%')
            status = 'FLAG'
...
def assemble_records(gb_data, bc_data, workbook_name, metadata, publication_date, download_date, ingested_at, existing, cfg, logger) -> list:
    records = []
    all_periods = sorted(set(gb_data.keys()) | set(bc_data.keys()), key=lambda p: (parse_period(p)[0], parse_period(p)[1] or 0))
    for label in all_periods:
        ...
        is_revised = prior_total is not None and total is not None and (abs(float(total) - float(prior_total)) > 1e-06)
        rec = {'variable_id': VARIABLE_ID, 'observation_period': label, 'observation_period_type': ptype, 'observation_year': year, 'observation_quarter': quarter, 'bar_demand_tonnes': gb.get('bar_demand_tonnes'), 'official_coin_demand_tonnes': gb.get('official_coin_demand_tonnes'), 'medals_imitation_coin_tonnes': gb.get('medals_imitation_coin_tonnes'), 'total_bar_and_coin_tonnes': total, 'named_country_total_tonnes': bc.get('named_country_total_tonnes'), 'other_and_stock_change_tonnes': bc.get('other_and_stock_change_tonnes'), 'world_total_bar_and_coin_sheet_tonnes': bc.get('world_total_bar_and_coin_sheet_tonnes'), 'unit': 'metric_tonnes', 'source_name': 'WGC_GDT', 'source_workbook': workbook_name, 'source_publication_date': publication_date, 'download_date': download_date, 'ingested_at': ingested_at, 'parser_version': PARSER_VERSION, 'underlying_providers': UNDERLYING_PROVIDERS, 'is_revised': is_revised, 'prior_total_bar_and_coin_tonnes': prior_total if is_revised else None, 'revision_reason': f'Value changed from {prior_total:.5f}t to {total:.5f}t in {workbook_name}' if is_revised else None, 'validation_status': None, 'availability_status': None, 'anomaly_notes': None}
        ...
        records.append(rec)
    return records
FIELDNAMES = ['variable_id', 'observation_period', 'observation_period_type', 'observation_year', 'observation_quarter', 'bar_demand_tonnes', 'official_coin_demand_tonnes', 'medals_imitation_coin_tonnes', 'total_bar_and_coin_tonnes', 'named_country_total_tonnes', 'other_and_stock_change_tonnes', 'world_total_bar_and_coin_sheet_tonnes', 'unit', 'source_name', 'source_workbook', 'source_publication_date', 'download_date', 'ingested_at', 'parser_version', 'underlying_providers', 'is_revised', 'prior_total_bar_and_coin_tonnes', 'revision_reason', 'validation_status', 'availability_status', 'anomaly_notes']
...
def append_to_csv(records: list, processed_path: Path):
    ...
    if processed_path.exists():
        with open(processed_path, newline='', encoding='utf-8') as f:
            for row in csv.DictReader(f):
                key = row.get('observation_period')
                if key:
                    existing[key] = row
    for record in records:
        existing[record['observation_period']] = record
    ordered = sorted(existing.values(), key=lambda row: (parse_period(row['observation_period'])[0], parse_period(row['observation_period'])[1] or 0))
...
def main():
    ...
    metadata = metadata_of_file(workbook_path)
    ...
    records = assemble_records(gb_data, bc_data, workbook_path.name, metadata, args.publication_date, args.download_date, ingested_at, existing, cfg, logger)
