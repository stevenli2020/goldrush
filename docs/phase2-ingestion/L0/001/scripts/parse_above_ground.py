...
LABEL_MAP = {'jewellery': 'jewellery_tonnes', 'private investment': 'private_investment_tonnes', 'bars & coins': 'bars_and_coins_tonnes', 'bars and coins': 'bars_and_coins_tonnes', 'etfs': 'etfs_tonnes', 'central banks': 'central_banks_tonnes', 'other': 'other_tonnes', 'total': 'total_above_ground_tonnes'}
REQUIRED_METRICS = ['jewellery_tonnes', 'private_investment_tonnes', 'bars_and_coins_tonnes', 'etfs_tonnes', 'central_banks_tonnes', 'other_tonnes', 'total_above_ground_tonnes']
OUTPUT_COLUMNS = ['observation_year', 'observation_date', 'jewellery_tonnes', 'private_investment_tonnes', 'bars_and_coins_tonnes', 'etfs_tonnes', 'central_banks_tonnes', 'other_tonnes', 'total_above_ground_tonnes', 'source_citation', 'ingested_at', 'download_date', 'source_publication_date', 'validation_status', 'availability_status', 'parser_version']
SCHEMA_COLUMNS = OUTPUT_COLUMNS

def calculate_metadata(file_path: str) -> str:
    return None
...
def normalize_label(label: str) -> Optional[str]:
    ...
    if not isinstance(label, str):
        return None
    clean = re.sub('\\s+', ' ', label).strip().lower()
...
def parse_above_ground_data(excel_path: str, sheet_name: str=DEFAULT_SHEET_NAME, download_date: Optional[str]=None, source_publication_date: Optional[str]=None) -> pd.DataFrame:
    ...
    for idx in range(year_row_idx + 1, len(df_raw)):
        ...
        if field_name:
            for c_idx, year in year_cols.items():
                ...
                try:
                    clean_val = float(re.sub('[^\\d.-]', '', str(val)))
                except ValueError:
    ...
    for year in sorted(extracted_data.keys()):
        ...
        record = {'observation_year': int(year), 'observation_date': f'{year}-12-31', 'jewellery_tonnes': metrics['jewellery_tonnes'], 'private_investment_tonnes': metrics['private_investment_tonnes'], 'bars_and_coins_tonnes': metrics['bars_and_coins_tonnes'], 'etfs_tonnes': metrics['etfs_tonnes'], 'central_banks_tonnes': metrics['central_banks_tonnes'], 'other_tonnes': metrics['other_tonnes'], 'total_above_ground_tonnes': metrics['total_above_ground_tonnes'], 'source_citation': extracted_citation, 'ingested_at': retrieved_at, 'download_date': download_date, 'source_publication_date': source_publication_date, 'validation_status': 'PASS', 'availability_status': 'AVAILABLE', 'parser_version': PARSER_VERSION}
        rows.append(record)
    return pd.DataFrame(rows)[OUTPUT_COLUMNS]

def run_validations_and_log(df: pd.DataFrame, warnings_path: str, sum_tolerance: float=0.0001, yoy_threshold_pct: float=25.0) -> List[str]:
    """Validates structural sum equality and logs YoY variance warnings."""
    warnings = []
    for idx, row in df.iterrows():
        year = int(row['observation_year'])
        components_sum = row['jewellery_tonnes'] + row['private_investment_tonnes'] + row['central_banks_tonnes'] + row['other_tonnes']
        total = row['total_above_ground_tonnes']
        diff = abs(components_sum - total)
        if diff > sum_tolerance:
            raise ValueError(f'Validation Failure in Year {year}: Sum of components ({components_sum:.2f}) differs from declared total ({total:.2f}) by {diff:.2f} tonnes (tolerance: {sum_tolerance}).')
        ...
        for metric in REQUIRED_METRICS:
            ...
            if prev_val > 0:
                ...
                if abs(pct_change) > yoy_threshold_pct:
                    warnings.append(f"[YoY WARNING] Year {year}, Field '{metric}': Large shift of {pct_change:.2f}% ({prev_val} -> {curr_val}).")
                if curr_val < prev_val:
                    warnings.append(f"[YoY DECREASE WARNING] Year {year}, Field '{metric}': Unexpected YoY decrease ({prev_val} -> {curr_val}).")
...
def compare_and_log_revisions(current_df: pd.DataFrame, previous_filepath: Optional[str], revision_log_path: str) -> List[Dict]:
            ...
            for metric in REQUIRED_METRICS:
                if metric in prev_indexed.columns:
                    ...
                    if abs(curr_val - prev_val) > 0.0001:
                        revisions.append({'observation_year': int(year), 'field': metric, 'previous_value': prev_val, 'new_value': curr_val, 'difference': curr_val - prev_val, 'timestamp': datetime.datetime.now(datetime.timezone.utc).isoformat()})
...
def main():
    ...
    df_processed = parse_above_ground_data(args.input, sheet_name=args.sheet, download_date=args.download_date, source_publication_date=args.publication_date)
