...
# Dynamically resolve project root (L0/001) and add 'scripts' to sys.path
...
from parse_above_ground import (
    SCHEMA_COLUMNS,
    parse_above_ground_data,
    run_validations_and_log,
    compare_revisions,
)

# Relative path based on your tree: data/above-ground-gold-stocks/2026/above-ground-gold-stocks.xlsx
WORKBOOK_PATH = str(
    PROJECT_ROOT / "data" / "above-ground-gold-stocks" / "2026" / "above-ground-gold-stocks.xlsx"
)
...
class TestAboveGroundCollector(unittest.TestCase):
    ...
    def test_parse_real_workbook_schema_and_provenance(self):
        ...
        for field in [
            ...
            self.assertFalse(
                df[field].isnull().all(), f"Field {field} should contain values"
            )
