...
class GoldRecyclingCollector:
    ...
    def _load_config(self, config_path: Path) -> None:
        try:
            ...
            if 'seed_csv' in paths:
                seed = Path(paths['seed_csv'])
                self.seed_path = seed if seed.is_absolute() else config_path.parent / seed
            if 'processed_output' in paths:
                output = Path(paths['processed_output'])
                self.output_path = output if output.is_absolute() else config_path.parent / output
        except Exception:
            pass

    def validate_and_apply_revisions(self, observations: List[Dict[str, Any]], existing_output_path: Optional[Union[str, Path]]=None) -> List[Dict[str, Any]]:
        """
        ...
        for obs in observations:
            ...
            if date in dated_records:
                ...
                if val != prev_val:
                    dated_records[date] = {'observation_date': date, 'value': val, 'unit': unit, 'frequency': freq, 'revision_status': 'REVISED', 'previous_value': prev_val, 'revision_notes': obs.get('revision_notes', 'Value revised against internal duplicate')}
            else:
                dated_records[date] = {'observation_date': date, 'value': val, 'unit': unit, 'frequency': freq, 'revision_status': rev_status}
    ...
    def run(self, publication_date: Optional[str]=None, source_file: Optional[Union[str, Path]]=None, is_live_source: bool=False, **kwargs: Any) -> Dict[str, Any]:
        ...
        payload = {'variable_id': self.variable_id, 'availability_status': availability_status, 'ingestion_metadata': {'publication_date': publication_date, 'retrieved_at': retrieved_at, 'source_file': str(target_file)}, 'observations': data, 'data': data}
