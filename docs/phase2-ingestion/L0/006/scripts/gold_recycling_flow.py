...
class GoldRecyclingCollector:
            ...
            if date in dated_records:
                ...
                if val != prev_val:
                    dated_records[date] = {
                        ...
                        "revision_notes": obs.get("revision_notes", "Value revised in subsequent update"),
                    }
            else:
                dated_records[date] = {
                    "observation_date": date,
                    ...
                    "revision_notes": obs.get("revision_notes"),
                }
    ...
    def extract_from_workbook(
        ...
        if path.suffix.lower() in [".xlsx", ".xlsm"]:
            ...
            # Deterministic fallback for the current WGC GDT layout. The
            # workbook uses row 5 for period headers and column B for labels.
            import pandas as pd
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                # Ambiguous row detection: skip rows with insufficient columns
                if len(row_vals) < 2:
                    continue

                parsed_date = parse_quarter_label(str(row_vals[0]))
                if not parsed_date:
                    # Retry second cell in case first column was an ambiguous label/header
                ...
                try:
                    ...
                    # Ambiguous/non-numeric row discarded
                    continue
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        # Seed fallback source must be marked STALE
