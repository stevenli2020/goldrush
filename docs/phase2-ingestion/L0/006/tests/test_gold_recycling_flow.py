...
# Resolve scripts directory dynamically relative to test file location
SCRIPTS_DIR = Path(__file__).resolve().parent.parent / "scripts"
if str(SCRIPTS_DIR) not in sys.path:
    sys.path.insert(0, str(SCRIPTS_DIR))

from parse_gold_recycling import (
    GoldRecyclingCollector,
    parse_quarter_label,
)
...
def test_ambiguous_row_matching_raises_error(config_file, tmp_path):
    ...
    # Create 2 matching candidate rows
...
def test_ambiguous_row_matching_raises_error(config_file, tmp_path):
    ...
    # Row 1: Add at least two quarter columns to satisfy header row detection
    ws.cell(row=1, column=1, value="Category")
    ws.cell(row=1, column=2, value="Q1'26")
