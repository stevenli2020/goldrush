    ...
    for sheet, raw in sheets.items():
        ...
        for _, row in raw.iloc[header_row + 1:].iterrows():
            ...
            if not isinstance(country, str) or not country.strip() or not pd.notna(value):
                continue
            try: tonnes = float(value)
            except (TypeError, ValueError): continue
            if tonnes < 0: raise ValueError(f"Negative holdings for {country}")
            rows.append({"variable_id":"L0-002", "country":country.strip(), "holdings_tonnes":tonnes,
                "unit":"metric_tonnes", "source_file":path.name, "source_publication_date":publication_date,
                "download_date":download_date, "workbook_sha256":digest,
                "ingested_at":datetime.now(timezone.utc).isoformat(), "validation_status":"PASS",
                "availability_status":"AVAILABLE", "parser_version":"1.0.0"})
    if not rows: raise ValueError("No official holdings rows found")
...
def main(argv=None):
    ...
    df=parse_file(a.input,a.publication_date,a.download_date); a.output.parent.mkdir(parents=True,exist_ok=True); df.to_csv(a.output,index=False); print(f"Parsed {len(df)} holdings records")
