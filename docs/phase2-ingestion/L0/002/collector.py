...
SYMBOL = "IL::RGV_REVS"
MULTIPLIER = 3.11034768e-5  # Troy ounces -> Metric Tonnes

PANEL_ENTITIES = {
    "US": "USA",
    "EA": "EZB",  # European Central Bank / Euro Area
    "CN": "CHN",
    "JP": "JPN",
    "CH": "CHE",
    "IMF": "IMF"
...
def run_collection(mock_mode: bool = False):
    ...
    for entity, country_code in PANEL_ENTITIES.items():
        try:
            if mock_mode:
                ...
                df = pd.DataFrame({
                    "date": [current_date_str, "2026-01-31"],
            ...
            # Freshness threshold: <= 150 days is FRESH; older (e.g. EA lag) is correctly flagged as STALE
            ...
            raw_responses_payload[country_code] = {
                "latest_observation_date": obs_date_str,
                "raw_value_troy_oz": raw_val,
                "metric_tonnes": round(tonnes, 3),
                "status": status,
                "age_days": age_days
            }
    ...
    raw_bundle = {
        "retrieved_at": run_id,
        "source": "IMF IFS via OpenBB",
        "query": SYMBOL,
        "entities": list(PANEL_ENTITIES.values()),
        "raw_response": raw_responses_payload
    }
    ...
    archive_bundle = {
        ...
        "unit": "metric_tonnes",
        "validation_status": "PASS" if aggregated_tonnes > 0 else "FAIL",
        "notes": "Execution completed successfully with robust 150-day freshness threshold"
    }
