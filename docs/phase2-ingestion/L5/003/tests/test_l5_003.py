...
def package(tmp_path, rows, *, dataset=parser.DATASET_ID, good_hash=True):
    ...
    manifest.write_text(json.dumps({
        "dataset_id": dataset, "source_url": collector.SOURCE_URL,
...
def test_valid_parse_missing_change_order_provenance_and_schema(tmp_path):
    ...
    assert records[2]["raw_sha256"] == hashlib.sha256(raw.read_bytes()).hexdigest()
...
def test_conflicting_duplicate_and_hash_or_dataset_rejected(tmp_path):
    raw, manifest = package(tmp_path, [row("2026-Q1", "57"), row("2026-Q1", "58")])
    with pytest.raises(ValueError, match="conflicting duplicate"): parser.parse(raw, manifest)
    raw, manifest = package(tmp_path / "hash", [row("2026-Q1", "57")], good_hash=False)
    with pytest.raises(ValueError, match="SHA-256"): parser.parse(raw, manifest)
    raw, manifest = package(tmp_path / "dataset", [row("2026-Q1", "57")], dataset="WRONG")
    with pytest.raises(ValueError, match="dataset_id"): parser.parse(raw, manifest)
...
def test_malformed_manifest_falls_back_and_prior_identity_is_checked(tmp_path, monkeypatch):
    ...
    rows = list(csv.DictReader(prior.open(newline="", encoding="utf-8"))); rows[0]["variable_id"] = "L7-003"
...
def test_cli_fallback_blocked_and_recovery(tmp_path, monkeypatch):
    ...
    assert fallback["availability_status"] == "STALE"; validate({**fallback, "usd_reserve_share_pct": float(fallback["usd_reserve_share_pct"]), "usd_share_change_qoq_pp": float(fallback["usd_share_change_qoq_pp"])})
    output.unlink()
