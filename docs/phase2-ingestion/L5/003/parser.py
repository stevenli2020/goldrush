...
STALE_DAYS = 200
FIELDS = ['variable_id', 'observation_date', 'usd_reserve_share_pct', 'usd_share_change_qoq_pp', 'unit', 'source_name', 'source_dataset_id', 'source_series_id', 'raw_path', 'manifest_path', 'retrieved_at', 'validation_status', 'availability_status', 'fallback_checked_at', 'parser_version']
MANIFEST_FIELDS = {'dataset_id', 'source_url', 'retrieved_at', 'raw_path', 'size_bytes', 'http_status', 'collector_version'}
...
def parse(raw_path: Path, manifest_path: Path, *, as_of: str | None=None, stale_days: int=STALE_DAYS) -> list[dict]:
    ...
    with raw_path.open(newline='', encoding='utf-8-sig') as handle:
        for row in csv.DictReader(handle):
            if not all((row.get(key) == value for key, value in {'COUNTRY': 'G001', 'INDICATOR': 'AFXRA', 'FXR_CURRENCY': 'CI_USD', 'TYPE_OF_TRANSFORMATION': 'SHRO_PT', 'FREQUENCY': 'Q'}.items())):
                continue
    ...
    for period in sorted(values):
        ...
        rows.append({'variable_id': VARIABLE_ID, 'observation_date': observation_date.isoformat(), 'usd_reserve_share_pct': value, 'usd_share_change_qoq_pp': change, 'unit': 'percent_and_percentage_points', 'source_name': 'IMF COFER', 'source_dataset_id': DATASET_ID, 'source_series_id': SERIES_ID, 'raw_path': str(raw_path), 'manifest_path': str(manifest_path), 'retrieved_at': manifest['retrieved_at'], 'validation_status': validation, 'availability_status': 'STALE' if (today - observation_date).days > stale_days else 'AVAILABLE', 'fallback_checked_at': None, 'parser_version': PARSER_VERSION})
    return rows
...
def validate_prior_row(row: dict) -> None:
    expected = {'variable_id': VARIABLE_ID, 'unit': 'percent_and_percentage_points', 'source_name': 'IMF COFER', 'source_dataset_id': DATASET_ID, 'source_series_id': SERIES_ID}
