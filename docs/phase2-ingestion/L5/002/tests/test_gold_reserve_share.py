...
class ReserveShareTests(unittest.TestCase):
    ...
    def test_extracts_share_and_holdings(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(result.iloc[0]['gold_share_of_reserves'], 0.25)
            self.assertEqual(result.iloc[0]['holdings_as_of'], '2026-06-30')
            self.assertEqual(set(result['panel']), {'left', 'right'})
    ...
    def test_rejects_share_out_of_bounds(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with self.assertRaises(ValueError):
                parse_file(p, '2026-08-20', '2026-08-21')
