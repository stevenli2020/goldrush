...
class SepParserTests(unittest.TestCase):
    def materials(self, directory, html=HTML, release="2026-06-17"):
        root = Path(directory)
        html_path = root / "sep.html"; html_path.write_text(html, encoding="utf-8")
        pdf_path = root / "sep.pdf"; pdf_path.write_bytes(b"%PDF-1.4 test")
        manifests = []
        for document_type, raw, url in [
            ("sep_html", html_path, "https://www.federalreserve.gov/monetarypolicy/fomcprojtabl20260617.htm"),
            ("sep_pdf", pdf_path, "https://www.federalreserve.gov/monetarypolicy/files/fomcprojtabl20260617.pdf"),
        ]:
            manifest = root / f"{document_type}.json"
            manifest.write_text(json.dumps({"document_type": document_type, "release_date": release,
                "meeting_date": release, "retrieved_at": "2026-08-24T00:00:00+00:00",
                "source_url": url, "raw_path": str(raw),
    ...
    def test_participant_reconciliation_and_median(self):
        with tempfile.TemporaryDirectory() as directory:
            rows = parse_sep(*self.materials(directory))
            totals = {h: sum(r["participant_count"] for r in rows if r["projection_horizon"] == h)
                      for h in {r["projection_horizon"] for r in rows}}
    ...
    def test_schema_and_provenance(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for row in rows: validate(row, schema)
            self.assertEqual(len(rows[0]["html_sha256"]), 64)

    def test_duplicate_rate_bin_fails(self):
        with tempfile.TemporaryDirectory() as directory:
            bad = HTML.replace("<tr><th>4.500</th>", "<tr><th>4.000</th>")
            with self.assertRaises(ValueError): parse_sep(*self.materials(directory, bad))
    ...
    def test_cli_blocked_and_prior_stale(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory); output = root / "out.csv"
            ...
            rows = parse_sep(*self.materials(directory)); write_csv(rows, output)
            ...
            result = subprocess.run([sys.executable, str(script), "--html-manifest", str(bad),
                                     "--pdf-manifest", str(bad), "--prior", str(output),
