...
FIELDS = [
    "variable_id", "sep_release_date", "meeting_date", "projection_horizon",
    "rate_bin_midpoint", "participant_count", "median_projected_rate", "unit",
    "publication_timestamp", "retrieved_at", "html_source_url", "pdf_source_url",
    "html_raw_path", "pdf_raw_path", "html_manifest_path", "pdf_manifest_path",
    "html_sha256", "pdf_sha256", "validation_status", "availability_status",
    "parser_version",
]
...
def publication_timestamp(text: str, release_date: str) -> str:
    match = re.search(r"For release at\s+(\d{1,2}):(\d{2})\s+([ap])\.m\.,\s+(EDT|EST)", text, re.I)
    if not match:
...
def parse_sep(html_manifest: Path, pdf_manifest: Path, *, stale_after_days: int = 120) -> list[dict[str, Any]]:
    ...
    distribution = next((table for table in soup.find_all("table")
                         if "Midpoint of target range or target level" in table.get_text(" ", strip=True)), None)
    ...
    median_row = next(([cell.get_text(" ", strip=True) for cell in tr.find_all(["th", "td"])]
                       for tr in median_table.find_all("tr")
                       if tr.find_all(["th", "td"]) and
                       tr.find_all(["th", "td"])[0].get_text(" ", strip=True) == "Federal funds rate"), None)
    ...
    for horizon in headers:
        for rate, count in sorted(by_horizon[horizon]):
            rows.append({
                "variable_id": VARIABLE_ID, "sep_release_date": release_date,
                "meeting_date": html_meta["meeting_date"], "projection_horizon": horizon,
                "rate_bin_midpoint": rate, "participant_count": count,
                "median_projected_rate": medians[horizon], "unit": "percent",
                "publication_timestamp": published, "retrieved_at": html_meta["retrieved_at"],
                "html_source_url": html_meta["source_url"], "pdf_source_url": pdf_meta["source_url"],
                "html_raw_path": str(html_path), "pdf_raw_path": pdf_meta["raw_path"],
                "html_manifest_path": str(html_manifest), "pdf_manifest_path": str(pdf_manifest),
                "html_sha256": html_meta["sha256"], "pdf_sha256": pdf_meta["sha256"],
                "validation_status": "PASS", "availability_status": availability,
...
def main(argv: list[str] | None = None) -> int:
    ...
    cli.add_argument("--prior", type=Path); cli.add_argument("--stale-after-days", type=int, default=120)
    ...
    try:
        ...
        try:
            ...
            if not rows: raise ValueError("no prior valid SEP output exists")
        except (OSError, ValueError) as fallback_exc:
            blocked(args.output, str(fallback_exc)); return 0
