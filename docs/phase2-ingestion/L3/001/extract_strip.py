...
def extract(source: Path, output: Path, sha256: str, manifest: str) -> int:
    ...
    for row in rows:
        ...
        if 1 <= months <= 12:
            normalized.append({
                ...
                "implied_rate_percent": f"{100.0 - float(row['settlement_price']):.4f}",
                "months_ahead": str(months),
...
if __name__ == "__main__":
    ...
    print(f"Wrote {extract(args.input, args.output, args.sha256, args.manifest)} strip rows")
