...
def parse(path: Path, max_months: int = 12) -> list[dict]:
    ...
    required = {"observation_date", "contract", "implied_rate_percent", "months_ahead", "source_pdf_sha256", "source_manifest"}
    ...
    for row in rows:
        ...
        if months < 1 or months > max_months or not 0 <= rate <= 20:
            continue
        grouped.setdefault(obs.isoformat(), []).append({"contract": row["contract"], "rate": rate, "months": months,
    ...
    for obs, contracts in sorted(grouped.items()):
        ...
        output.append({"variable_id": "L3-001", "observation_date": obs,
                       "path_average_percent": value, "contracts_used": len(contracts),
                       "horizon_months": max(item["months"] for item in contracts),
                       "source_pdf_sha256": contracts[0]["source_pdf_sha256"],
                       "source_manifest": contracts[0]["source_manifest"],
                       "unit": "percent_per_annum", "source_series_id": "CME_FED_FUNDS_FUTURES_STRIP",
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(); p.add_argument("--input", type=Path, required=True); p.add_argument("--output", type=Path, required=True); p.add_argument("--max-months", type=int, default=12)
    args = p.parse_args(); output = parse(args.input, args.max_months); args.output.parent.mkdir(parents=True, exist_ok=True)
