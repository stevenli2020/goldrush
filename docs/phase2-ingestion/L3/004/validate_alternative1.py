...
SUM_TOLERANCE = 0.001
MAX_BUCKET_ERROR = 0.10
MAX_TVD = 0.10
MATERIAL_PROBABILITY = 0.001
OFFICIAL_NAME = re.compile(r"^FedMeeting_(\d{8})_downloaded_\d{4}-\d{2}-\d{2}\.csv$")
OFFICIAL_BUCKET = re.compile(r"^\((\d+)-(\d+)\)$")
PACKAGE_BUCKET = re.compile(r"^(\d+(?:\.\d+)?)%-(\d+(?:\.\d+)?)%$")
...
def compare_distributions(official: dict[str, float], calculated: dict[str, float]) -> dict:
    ...
    missing_material = sorted(
        bucket for bucket, probability in official.items()
        if probability >= MATERIAL_PROBABILITY and bucket not in calculated
    )
    maximum_error = max(errors.values(), default=0.0)
    tvd = 0.5 * sum(errors.values())
    checks = {
        "official_sum_within_tolerance": abs(official_sum - 1.0) <= SUM_TOLERANCE,
        "calculated_sum_within_tolerance": abs(calculated_sum - 1.0) <= SUM_TOLERANCE,
        "modal_outcome_agrees": official_modal == calculated_modal,
        "maximum_error_within_threshold": maximum_error <= MAX_BUCKET_ERROR,
        "tvd_within_threshold": tvd <= MAX_TVD,
        "no_material_official_bucket_missing": not missing_material,
    }
    return {
        "official_probability_sum": official_sum,
        "calculated_probability_sum": calculated_sum,
        "official_modal_outcome": official_modal,
        "calculated_modal_outcome": calculated_modal,
        "official_bucket_coverage": sorted(official),
        "calculated_bucket_coverage": sorted(calculated),
        "missing_material_official_buckets": missing_material,
        "maximum_absolute_bucket_error": maximum_error,
...
def build_report(official_dir: Path, manifests: list[Path]) -> dict:
    ...
    for manifest_path in manifests:
        ...
        for sequence, meeting in enumerate(tree[:3], start=1):
            ...
            calculated = {
                bucket_key(inputs["target_lower"] + moves * 0.25, inputs["target_upper"] + moves * 0.25): probability
                for moves, probability in meeting["distribution"].items()
            }
            metrics = compare_distributions(official, calculated)
            buckets = sorted(set(official) | set(calculated))
            comparisons.append({
                "observation_date": inputs["observation_date"].isoformat(),
                "meeting_date": meeting_date.isoformat(),
                "meeting_sequence": sequence,
                "production_scope": sequence <= 2,
                "contract": meeting["contract"],
                "conditional_transition": {str(key): value for key, value in meeting["conditional_transition"].items()},
                "official_file_path": str(official_path),
                "official_file_sha256": sha256_file(official_path),
                "source_manifest_path": str(manifest_path),
                "source_manifest_sha256": sha256_file(manifest_path),
                "official_distribution": official,
                "calculated_distribution": calculated,
                "bucket_differences": {bucket: calculated.get(bucket, 0.0) - official.get(bucket, 0.0) for bucket in buckets},
                **metrics,
            })
    ...
    return {
        "comparison": "L3-004 cumulative probability tree validation",
        "package_name": "cme-fedwatch", "package_version": PACKAGE_VERSION,
        "calculation_method": production.METHOD,
        "thresholds": {"probability_sum_tolerance": SUM_TOLERANCE,
            "maximum_absolute_bucket_error": MAX_BUCKET_ERROR,
            "maximum_total_variation_distance": MAX_TVD,
            "material_official_probability": MATERIAL_PROBABILITY},
        "comparisons": sorted(comparisons, key=lambda item: (item["observation_date"], item["meeting_date"])),
        "production_scope_gate_pass": production_gate,
        "recursive_validation_pass": recursive_gate,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({"production_scope_gate_pass": report["production_scope_gate_pass"],
        "recursive_validation_pass": report["recursive_validation_pass"], "report": str(args.report)}))
    return 0
