...
def collect(
    ...
    *,
    ...
    for series_id in ("EFFR", "DFEDTARL", "DFEDTARU"):
        record = fred.fetch_series(
            series_id, api_key, fred_raw_dir, fred_manifest_dir,
            observation_start=start, observation_end=actual_date.isoformat(), force=True,
        )
        fred_records[series_id] = record

    package_version = version("cme-fedwatch")
    schedule_record = {
        "package_name": "cme-fedwatch",
        "package_version": package_version,
        "snapshot_for": actual_date.isoformat(),
        "schedule_status": schedule_status(actual_date),
        "meetings": [meeting.isoformat() for meeting in FOMC_MEETINGS],
    ...
    manifest = {
        ...
        "observation_date": actual_date.isoformat(),
        "retrieved_at": retrieved.isoformat(),
        "cme": {
            "source_url": cme_url,
            "product_id": "305/FUT",
            "raw_path": str(cme_path),
            "sha256": sha256_bytes(content),
            "trade_date": actual_date.isoformat(),
            "report_type": cme_payload.get("reportType"),
            "update_time": cme_payload.get("updateTime"),
        },
        "fred": fred_records,
        "schedule": {
            "raw_path": str(schedule_path),
            "sha256": sha256_bytes(schedule_path.read_bytes()),
            "schedule_status": schedule_record["schedule_status"],
            "source_url": schedule_record["source_url"],
        },
        "package_name": "cme-fedwatch",
        "package_version": package_version,
    }
