...
FIELDS = [
    "variable_id", "observation_date", "meeting_date", "contract",
    "target_rate_lower_pct", "target_rate_upper_pct", "probability",
    "probability_sum_original", "calculation_method", "source_name",
    "source_settlement_identifiers", "effr_observation_date", "effr_pct",
    "current_target_lower_pct", "current_target_upper_pct", "retrieved_at",
    "source_manifest_path", "source_manifest_sha256", "cme_raw_path", "cme_raw_sha256",
    "effr_raw_path", "effr_raw_sha256", "target_lower_raw_path", "target_lower_raw_sha256",
    "target_upper_raw_path", "target_upper_raw_sha256", "schedule_path", "schedule_sha256",
    "package_version", "parser_version", "is_revision", "prior_probability",
...
def load_and_verify(manifest_path: Path) -> dict:
    ...
    if sha256_file(cme_path) != cme["sha256"]:
        raise ValueError("CME raw hash mismatch")
    schedule = manifest["schedule"]
    schedule_path = Path(schedule["raw_path"])
    if sha256_file(schedule_path) != schedule["sha256"]:
    ...
    for series_id in ("EFFR", "DFEDTARL", "DFEDTARU"):
        ...
        if sha256_file(Path(record["raw_path"])) != record["sha256"]:
            raise ValueError(f"{series_id} raw hash mismatch")
    return manifest
...
def source_inputs(manifest_path: Path) -> dict:
    ...
    if not math.isclose(target_upper - target_lower, 0.25, abs_tol=1e-9):
    ...
    return {"manifest":manifest, "observation_date":observation_date, "settlements":settlements,
        "effr_date":effr_date, "effr":effr, "target_lower":target_lower, "target_upper":target_upper,
        "schedule":schedule, "meetings":meetings[:3], "schedule_state":state}
...
def calculate_tree(inputs: dict) -> list[dict]:
    ...
    for index, meeting in enumerate(meetings):
        ...
        results.append({"meeting_date":meeting, "contract":contract_code(meeting),
            "distribution":cumulative, "conditional_transition":transition,
            "source_settlement_identifiers":source_ids, "expected_post_rate":post_rate})
        pre_rate = post_rate
    return results
...
def prior_values(path: Path | None) -> dict[tuple[str, str, float, float], float]:
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        return {(row["observation_date"],row["meeting_date"],float(row["target_rate_lower_pct"]),float(row["target_rate_upper_pct"])):float(row["probability"])
            for row in csv.DictReader(handle) if row.get("validation_status") in {"PASS","FLAG"}}
...
def parse_manifest(manifest_path: Path, prior_path: Path | None = None, production_meetings: int = 2, stale_after_days: int = 3) -> list[dict]:
    ...
    for meeting in tree[:production_meetings]:
        ...
        for moves, probability in sorted(meeting["distribution"].items()):
            ...
            fred=manifest["fred"]
            rows.append({
                "variable_id":VARIABLE_ID,"observation_date":inputs["observation_date"].isoformat(),
                "meeting_date":meeting["meeting_date"].isoformat(),"contract":meeting["contract"],
                "target_rate_lower_pct":lower,"target_rate_upper_pct":upper,"probability":probability,
                "probability_sum_original":total,"calculation_method":METHOD,"source_name":SOURCE_NAME,
                "source_settlement_identifiers":"|".join(meeting["source_settlement_identifiers"]),
                "effr_observation_date":inputs["effr_date"].isoformat(),"effr_pct":inputs["effr"],
                "current_target_lower_pct":inputs["target_lower"],"current_target_upper_pct":inputs["target_upper"],
                "retrieved_at":manifest["retrieved_at"],"source_manifest_path":str(manifest_path),
                "source_manifest_sha256":manifest_hash,"cme_raw_path":manifest["cme"]["raw_path"],
                "cme_raw_sha256":manifest["cme"]["sha256"],"effr_raw_path":fred["EFFR"]["raw_path"],
                "effr_raw_sha256":fred["EFFR"]["sha256"],"target_lower_raw_path":fred["DFEDTARL"]["raw_path"],
                "target_lower_raw_sha256":fred["DFEDTARL"]["sha256"],"target_upper_raw_path":fred["DFEDTARU"]["raw_path"],
                "target_upper_raw_sha256":fred["DFEDTARU"]["sha256"],"schedule_path":manifest["schedule"]["raw_path"],
                "schedule_sha256":manifest["schedule"]["sha256"],"package_version":manifest["package_version"],
                "parser_version":PARSER_VERSION,"is_revision":prior is not None and not math.isclose(prior,probability,abs_tol=1e-12),
                "prior_probability":prior if prior is not None and not math.isclose(prior,probability,abs_tol=1e-12) else None,
...
def carry_forward(prior_path: Path) -> list[dict]:
    ...
    for row in result: row["availability_status"]="STALE"
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    cli.add_argument("--manifest",type=Path); cli.add_argument("--prior",type=Path)
    cli.add_argument("--output",type=Path,default=Path("docs/phase2-ingestion/L3/004/data/processed/L3_004_probabilities.csv"))
    args=cli.parse_args(argv)
    try:
        if args.manifest: rows=parse_manifest(args.manifest,args.prior)
        elif args.prior: rows=carry_forward(args.prior)
        else: raise ValueError("provide --manifest or --prior")
        ...
        if args.prior:
            ...
            path=write_blocked(args.output,str(exc)); print(json.dumps({"status":"BLOCKED","status_path":str(path)})); return 0
    write_csv(rows,args.output); print(json.dumps({"rows":len(rows),"observation_date":rows[-1]["observation_date"]})); return 0
