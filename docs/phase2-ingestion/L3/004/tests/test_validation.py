...
def test_filename_and_basis_point_range_parsing():
    ...
    with pytest.raises(ValueError):
        module.meeting_date_from_filename(Path('meeting.csv'))
    with pytest.raises(ValueError):
        module.official_bucket('(375-350)')
...
def test_official_csv_rejects_bad_header(tmp_path):
    ...
    with pytest.raises(ValueError, match='Date header'):
        module.load_official(path, date(2026, 8, 21))

def test_comparison_thresholds_and_missing_bucket():
    passing = module.compare_distributions({'(350-375)': 0.6, '(375-400)': 0.4}, {'(350-375)': 0.5, '(375-400)': 0.5})
    assert passing['maximum_absolute_bucket_error'] == pytest.approx(0.1)
    assert passing['total_variation_distance'] == pytest.approx(0.1) and passing['pass']
    failing = module.compare_distributions({'(350-375)': 0.6, '(375-400)': 0.4}, {'(375-400)': 0.2, '(400-425)': 0.8})
