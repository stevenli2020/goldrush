...
def fixture(tmp_path, state="ok", retrieved_at="2026-08-24T02:00:00+00:00"):
    ...
    cme.write_text(json.dumps({"tradeDate":"08/21/2026","settlements":[
        {"month":"SEP 26","settle":"96.3250"},{"month":"OCT 26","settle":"96.2650"},
        {"month":"NOV 26","settle":"96.2150"},{"month":"DEC 26","settle":"96.1400"}]}),encoding="utf-8")
    fred={}
    for series,value,obs in [("EFFR","3.63","2026-08-20"),("DFEDTARL","3.50","2026-08-21"),("DFEDTARU","3.75","2026-08-21")]:
        path=tmp_path/f"{series}.json"; path.write_text(json.dumps({"observations":[{"date":obs,"value":value}]}),encoding="utf-8")
        fred[series]={"series_id":series,"raw_path":str(path),"sha256":digest(path)}
    ...
    manifest.write_text(json.dumps({"variable_id":"L3-004","observation_date":"2026-08-21","retrieved_at":retrieved_at,
        "cme":{"raw_path":str(cme),"sha256":digest(cme),"trade_date":"2026-08-21"},"fred":fred,
        "schedule":{"raw_path":str(schedule),"sha256":digest(schedule)},"package_version":"0.1.3"}),encoding="utf-8")
    return manifest,cme,schedule


def test_cumulative_tree_matches_expected_and_schema(tmp_path):
    manifest,_,_=fixture(tmp_path); rows=module.parse_manifest(manifest)
    ...
    schema=json.loads(SCHEMA_PATH.read_text()); validator=Draft7Validator(schema,format_checker=FormatChecker())
...
def test_raw_hash_and_settlement_date_validation(tmp_path):
    manifest,cme,_=fixture(tmp_path/"hash"); cme.write_text("{}",encoding="utf-8")
    with pytest.raises(ValueError,match="hash mismatch"): module.parse_manifest(manifest)
    manifest,cme,_=fixture(tmp_path/"date"); payload=json.loads(cme.read_text()); payload["tradeDate"]="08/20/2026"; cme.write_text(json.dumps(payload))
    record=json.loads(manifest.read_text()); record["cme"]["sha256"]=digest(cme); manifest.write_text(json.dumps(record))
    with pytest.raises(ValueError,match="settlement date"): module.parse_manifest(manifest)
...
def test_expired_schedule_blocks_even_with_prior(tmp_path):
    good,_,_=fixture(tmp_path/"good"); prior=tmp_path/"prior.csv"; module.write_csv(module.parse_manifest(good),prior)
    expired,_,_=fixture(tmp_path/"expired",state="expired"); output=tmp_path/"out.csv"
...
def test_unchanged_replay_and_value_revision(tmp_path):
    manifest,cme,_=fixture(tmp_path); first=module.parse_manifest(manifest); prior=tmp_path/"prior.csv"; module.write_csv(first,prior)
    assert not any(row["is_revision"] for row in module.parse_manifest(manifest,prior))
    payload=json.loads(cme.read_text()); payload["settlements"][0]["settle"]="96.3200"; cme.write_text(json.dumps(payload))
    record=json.loads(manifest.read_text()); record["cme"]["sha256"]=digest(cme); manifest.write_text(json.dumps(record))
...
def test_stale_fallback_blocked_and_recovery(tmp_path):
    manifest,_,_=fixture(tmp_path); output=tmp_path/"out.csv"
