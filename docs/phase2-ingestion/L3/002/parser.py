...
CONTRACT_RE = re.compile(r"^ZQ[FGHJKMNQUVXZ]\d{2}$")
REQUIRED = {"observation_date", "contract", "settlement_price", "expiry_date"}
FIELDS = [
    "variable_id", "observation_date", "contract", "settlement_price",
    "implied_policy_rate_pct", "expiry_date", "months_ahead", "curve_position",
    "unit", "source_name", "source_url", "source_pdf_path", "source_manifest_path",
    "source_pdf_sha256", "retrieved_at", "formula_version", "parser_version",
    "is_revised", "prior_settlement_price", "validation_status", "availability_status",
]
...
def load_manifest(path: Path, source_pdf: Path) -> dict:
    ...
    if manifest.get("sha256") != digest:
        raise ValueError("source PDF hash does not match manifest")
    for field in ("source_url", "retrieved_at", "observation_date"):
...
def prior_values(path: Path | None) -> dict[tuple[str, str], float]:
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        return {
            (row["observation_date"], row["contract"]): float(row["settlement_price"])
            for row in csv.DictReader(handle)
            if row.get("validation_status") == "PASS"
        }


def parse_curve(source: Path, manifest_path: Path, source_pdf: Path,
                prior_path: Path | None = None, stale_after_days: int = 3) -> list[dict]:
    ...
    for row in input_rows:
        ...
        if obs.isoformat() != manifest["observation_date"]:
            raise ValueError("settlement observation date does not match manifest")
        if expiry <= obs:
            continue
        key = (obs.isoformat(), contract)
        normalized = {"observation_date": obs, "contract": contract,
                      "settlement_price": settlement, "expiry_date": expiry}
    ...
    for item in ordered:
        ...
        rows.append({
            ...
            "implied_policy_rate_pct": rate,
            "expiry_date": expiry.isoformat(),
            "months_ahead": (expiry.year - obs.year) * 12 + expiry.month - obs.month + 1,
            "curve_position": positions[obs.isoformat()],
            "unit": "percent_per_annum",
            "source_name": "CME 30-Day Fed Funds futures",
            "source_url": manifest["source_url"],
            "source_pdf_path": str(source_pdf),
            "source_manifest_path": str(manifest_path),
            "source_pdf_sha256": manifest["sha256"],
            "retrieved_at": manifest["retrieved_at"],
            "formula_version": FORMULA_VERSION,
            "parser_version": PARSER_VERSION,
            "is_revised": prior is not None and prior != settlement,
            "prior_settlement_price": prior if prior is not None and prior != settlement else None,
            "validation_status": "PASS" if 0 <= rate <= 20 else "FLAG",
            "availability_status": "STALE" if age > stale_after_days else "AVAILABLE",
        })
    return rows
...
def write_blocked(output: Path, reason: str) -> Path:
    ...
    path.write_text(json.dumps({"variable_id": VARIABLE_ID, "status": "BLOCKED",
