...
def build(path: Path, source: str) -> list[dict]:
    ...
    out=[]
    for r in rows:
        if REQUIRED-r.keys() or r['variable_id']!='L4-004' or r['unit']!='percent' or r['validation_status']!='PASS': raise ValueError('invalid L4-004 row')
        out.append({'variable_id':'L4-004','observation_timestamp':f"{r['observation_date']}T00:00:00Z",'value':float(r['value']),'unit_or_scale':'percent','availability_status':r['availability_status'],'source_reference':source,'quality_flag':r['validation_status']})
    return out
