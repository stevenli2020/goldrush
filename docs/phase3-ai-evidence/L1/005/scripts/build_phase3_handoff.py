"""Build the canonical Phase 3 handoff for L1-005."""
...
def load_rows(path: Path) -> list[dict[str, str]]:
    ...
    if not reader.fieldnames or not required.issubset(reader.fieldnames) or not rows:
        raise ValueError("L1-005 processed input is invalid or empty")
    if any(r["variable_id"] != "L1-005" or r["availability_status"] == "BLOCKED" for r in rows):
        raise ValueError("L1-005 input contains an invalid or blocked observation")
    return rows

def build_handoff(rows: list[dict[str, str]], source_reference: str) -> list[dict[str, object]]:
    return [{"variable_id": "L1-005", "observation_timestamp": f"{r['observation_date']}T00:00:00Z",
