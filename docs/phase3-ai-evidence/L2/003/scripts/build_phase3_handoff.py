"""Build the canonical Phase 3 handoff for L2-003."""
...
def load_rows(path: Path) -> list[dict[str, str]]:
    ...
    required = {"variable_id", "observation_date", "usd_cny", "unit", "validation_status", "availability_status"}
    if not reader.fieldnames or not required.issubset(reader.fieldnames) or not rows:
        raise ValueError("L2-003 processed input is invalid or empty")
    if any(r["variable_id"] != "L2-003" or r["unit"] != "cny_per_usd" for r in rows):
        raise ValueError("L2-003 input contains an invalid exchange-rate record")
    return rows

def build_handoff(rows: list[dict[str, str]], source_reference: str) -> list[dict[str, object]]:
    return [{"variable_id": "L2-003", "observation_timestamp": f"{r['observation_date']}T00:00:00Z",
             "value": float(r["usd_cny"]), "unit_or_scale": "cny_per_usd",
