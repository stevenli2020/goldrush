"""Build the canonical Phase 3 handoff for L2-001."""
...
def load_rows(path: Path) -> list[dict[str, str]]:
    ...
    required = {"variable_id", "observation_date", "dxy_close", "canonical_field", "unit", "validation_status", "availability_status"}
    if not reader.fieldnames or not required.issubset(reader.fieldnames) or not rows:
        raise ValueError("L2-001 processed input is invalid or empty")
    if any(r["variable_id"] != "L2-001" or r["canonical_field"] != "dxy_close" for r in rows):
        raise ValueError("L2-001 input contains an invalid canonical field")
    return rows

def build_handoff(rows: list[dict[str, str]], source_reference: str) -> list[dict[str, object]]:
    return [{"variable_id": "L2-001", "observation_timestamp": f"{r['observation_date']}T00:00:00Z",
             "value": float(r["dxy_close"]), "unit_or_scale": "index", "availability_status": r["availability_status"],
