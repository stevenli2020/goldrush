"""Build the canonical Phase 3 handoff for L3-001."""
...
def load_rows(path: Path) -> list[dict[str, str]]:
    ...
    required = {"variable_id", "observation_date", "path_average_percent", "unit", "validation_status", "availability_status"}
    if not reader.fieldnames or not required.issubset(reader.fieldnames) or not rows:
        raise ValueError("L3-001 processed input is invalid or empty")
    if any(r["variable_id"] != "L3-001" or r["unit"] != "percent_per_annum" for r in rows):
        raise ValueError("L3-001 input contains an invalid path-rate record")
    return rows

def build_handoff(rows: list[dict[str, str]], source_reference: str) -> list[dict[str, object]]:
    return [{"variable_id": "L3-001", "observation_timestamp": f"{r['observation_date']}T00:00:00Z",
             "value": float(r["path_average_percent"]), "unit_or_scale": "percent_per_annum",
