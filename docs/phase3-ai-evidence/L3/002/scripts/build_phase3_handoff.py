"""Build the canonical Phase 3 handoff for L3-002."""
...
def load_rows(path: Path) -> list[dict[str, str]]:
    ...
    required = {"variable_id", "observation_date", "contract", "implied_policy_rate_pct", "unit", "validation_status", "availability_status"}
    if not reader.fieldnames or not required.issubset(reader.fieldnames) or not rows:
        raise ValueError("L3-002 processed input is invalid or empty")
    if any(r["variable_id"] != "L3-002" or r["unit"] != "percent_per_annum" for r in rows):
        raise ValueError("L3-002 input contains an invalid curve record")
    return rows

def build_handoff(rows: list[dict[str, str]], source_reference: str) -> list[dict[str, object]]:
    return [{"variable_id": "L3-002", "observation_timestamp": f"{r['observation_date']}T00:00:00Z",
             "value": float(r["implied_policy_rate_pct"]), "unit_or_scale": "percent_per_annum",
             "contract": r["contract"], "availability_status": r["availability_status"],
