"""Reader and status-only method for approved L5-006 MVP treatment."""
...
def read_l5_006(path: str | Path) -> list[dict[str, Any]]:
    return read_structure(path, VARIABLE_ID, {UNIT}, set(), reject_duplicate_rows=False)


def status_only_l5_006(record: Any, horizon: str, context: Any = None) -> dict[str, Any]:
    return _status_only(record, horizon, VARIABLE_ID, UNIT, context, CONTEXT_KEY, "P")


status_only_method = status_only_l5_006
